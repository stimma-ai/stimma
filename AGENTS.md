...
- **Package**: A deliverable: a `.stimmapackage` directory bundle (container Asset type `package`) holding members (library assets by content hash), recipe runs (derivative file trees), extras, and a cover (`index.html`). Built by `packages/bundle.py`, exported as a zip or a single HTML file, rebuildable when a member's Asset gets a new revision.
- **Recipe**: Deterministic code (built-in in `packages/builtin/`, or `recipes/*.py` in a stimpack) that turns assets with declared roles plus parameters into a file tree inside a package. Never calls a model, tool or the network; judgment enters as parameters. See `packages/recipes.py` and `docs/STIMPACK_AUTHORING.md`.
- **Cover**: A package's front page: arbitrary HTML/CSS/JS plus kit elements (`stimma-media`, `stimma-files`, `stimma-compare`, `stimma-grid`, `stimma-appearance`, `stimma-sizes`, `stimma-columns`) that reference package content by ref. The cover is authored by the agent; the auto cover is a plain fallback. `stimma-section page` groups content into landscape PDF pages; `layout="single|pair|stack"` shares media composition with responsive HTML. The renderer always adds the kit’s hairline and “Made with Stimma” footer once at the end of HTML and on every PDF slide; README and run files carry no generated-by branding. Static-first: expanded server-side so it reads without scripts, from a double-clicked file, and in the thumbnail renderer. See `packages/cover.py`.
