...
- **Cover**: A package's front page: arbitrary HTML/CSS/JS plus kit elements (`stimma-media`, `stimma-files`, `stimma-compare`, `stimma-grid`, `stimma-appearance`, `stimma-sizes`, `stimma-columns`) that reference package content by ref. The cover is authored by the agent; the auto cover is a plain fallback. `stimma-section page` groups content into landscape PDF pages; `layout="single|pair|stack"` shares media composition with responsive HTML. The renderer always adds the kit’s hairline and “Made with Stimma” footer, the package’s single brand moment; README and run files carry no generated-by branding. Static-first: expanded server-side so it reads without scripts, from a double-clicked file, and in the thumbnail renderer. See `packages/cover.py`.
