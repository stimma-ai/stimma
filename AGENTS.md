...
- **Cover**: A package's front page: arbitrary HTML/CSS/JS plus kit elements (`stimma-media`, `stimma-files`, `stimma-compare`, `stimma-grid`) that reference members by ref. Static-first: expanded server-side so it reads without scripts, from a double-clicked file, and in the thumbnail renderer. See `packages/cover.py`.
