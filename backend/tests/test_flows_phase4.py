...
Covers the Phase 4 exit gate from docs/FLOWS_DEV_PLAN.md §Phase 4:
  ...
  2. Error tasks surface for all failure categories, all actions
     (retry, skip, edit_flow) work, skip rejected for
     non-loop equations.
  3. HITL results durable across edits and forks. Recovery on rename works.
  4. Cross-flow task listing works (with a Phase-6 perf TODO
     acknowledged).
...
Tests are hermetic: real per-flow SQLite + fresh equation store + mock
...
from flow_runtime.engine import (
    _error_actions_for,
    _is_inside_foreach_iteration,
    _iteration_prefix_for,
)
from flow_runtime.media_arg import Media
from flow_runtime.summary import include_equation_in_status_summary
from routes.flows import _json_safe_flow_value
...
class TestHelpers:
    ...
    def test_json_safe_flow_value_serializes_media_wrappers(self):
        media = Media(42, path="/tmp/render.png", filename="render.png")

        assert _json_safe_flow_value(
            {
                media: {"asset": media},
                "items": [media, (media,)],
                "plain": "ok",
...
async def test_select_task_payload(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        101, db_path, flow_callable=r, evaluators=_mock_evaluators(), store=store,
    )
...
async def test_approve_task_payload(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        102, db_path, flow_callable=r, evaluators=_mock_evaluators(tool_value="fancy_asset"), store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    rows = await _wait_for_pending_tasks(db_path, 1)
...
async def test_select_resolution_unblocks_downstream(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        110, db_path, flow_callable=r, evaluators=_mock_evaluators(), store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    rows = await _wait_for_pending_tasks(db_path, 1)
...
async def test_approve_accept_completes_equation(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        111, db_path, flow_callable=r, evaluators=_mock_evaluators(tool_value="art1"), store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    rows = await _wait_for_pending_tasks(db_path, 1)
...
async def test_error_task_created_on_failure(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        120, db_path, flow_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_error_task_metadata_per_failure_category(isolated_store_and_db, category):
    ...
    runtime = FlowRuntime(
        200 + hash(category) % 1000, db_path, flow_callable=r,
...
async def test_error_task_skip_rejected_for_non_loop(isolated_store_and_db):
    ...
    reg = _mock_evaluators(fail_keys={"r/tool$0"}, fail_category="tool_error")
    runtime = FlowRuntime(
        121, db_path, flow_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_error_task_retry(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        122, db_path, flow_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    tasks = await _wait_for_pending_tasks(db_path, 1)
...
async def test_error_task_skip_in_foreach(isolated_store_and_db):
    ...
    @flow(inputs={"names": dsl_input("list[str]")})
    def r(names):
    ...
    runtime = FlowRuntime(
        123, db_path, flow_callable=r, inputs={"names": ["A", "B", "C"]},
...
@pytest.mark.asyncio
async def test_error_task_edit_flow_is_noop_on_runtime(isolated_store_and_db):
    db_path, store = isolated_store_and_db
    ...
    reg = _mock_evaluators(fail_keys={"r/tool$0"}, fail_category="tool_error")
    runtime = FlowRuntime(
        125, db_path, flow_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    tasks = await _wait_for_pending_tasks(db_path, 1)
    await runtime.resolve_error_task(tasks[0]["task_id"], "edit_flow")
    await runtime.stop()
...
async def test_hitl_survives_unrelated_edit(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        130, db_path, flow_callable=r_before,
...
async def test_hitl_reuse_on_fork(tmp_path):
    ...
    create_flow_state_db(parent_db)
    ...
    runtime1 = FlowRuntime(
        900, parent_db, flow_callable=r, evaluators=_mock_evaluators(), store=store,
    )
    ...
    runtime2 = FlowRuntime(
        901, fork_db, flow_callable=r, evaluators=_mock_evaluators(), store=store,
    )
...
async def test_hitl_recovery_on_function_rename(isolated_store_and_db):
    ...
    runtime1 = FlowRuntime(
        131, db_path, flow_callable=r_before,
        evaluators=_mock_evaluators(), store=store,
    )
    runtime1.build_initial_graph()
    await runtime1.start()
    ...
    runtime2 = FlowRuntime(
        132, db_path, flow_callable=r_renamed,
        evaluators=_mock_evaluators(), store=store,
    )
    runtime2.build_initial_graph()
    await runtime2.start()
    ...
    # The renamed flow's HITL equation should be COMPLETED via the
...
async def test_idempotent_resolution_under_concurrent_invalidation(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        140, db_path, flow_callable=r, evaluators=_mock_evaluators(), store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    rows = await _wait_for_pending_tasks(db_path, 1)
...
async def test_unblocks_most_work_calculation(isolated_store_and_db):
    ...
    runtime = FlowRuntime(
        150, db_path, flow_callable=r, evaluators=_mock_evaluators(), store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_unknown_error_action_rejected(isolated_store_and_db):
    ...
    reg = _mock_evaluators(fail_keys={"r/tool$0"}, fail_category="tool_error")
    runtime = FlowRuntime(
        160, db_path, flow_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    tasks = await _wait_for_pending_tasks(db_path, 1)
...
async def test_ws_events_on_hitl_lifecycle(isolated_store_and_db):
    ...
    assert "flow_task_created" in names
    assert "flow_task_resolved" in names
    created = next(p for n, p in cap.events if n == "flow_task_created")
    assert created["task_type"] == "select"
    assert created["flow_id"] == 170
...
@pytest.mark.asyncio
async def test_http_list_tasks_for_flow(client, db_session):
    """GET /api/flows/:id/tasks returns pending tasks for one flow."""
    # Create a flow row via the normal API.
    resp = await client.post("/api/flows", json={"name": "phase4-list"})
    assert resp.status_code == 200
    flow_id = resp.json()["id"]

    # Populate its state.db directly.
    from flow_runtime import get_flow_state_db_path
    ...
    r = await client.get(f"/api/flows/{flow_id}/tasks")
    assert r.status_code == 200, r.text
    body = r.json()
    assert len(body) == 1
    assert body[0]["task_id"] == f"{flow_id}:{task_id}"
    assert body[0]["task_type"] == "select"
    assert body[0]["flow_id"] == flow_id
...
async def test_http_resolve_rejects_invalid_shape(client, db_session):
    ...
    r1 = (await client.post("/api/flows", json={"name": "x-validate"})).json()
    from flow_runtime import get_flow_state_db_path
    from flow_runtime.graph import Equation, EquationStatus as S, EquationType as ET
    state_db = get_flow_state_db_path(r1["id"])
    eq = Equation(
        key="r/hitl.select$0",
        equation_type=ET.HITL,
...
async def test_http_resolve_via_registry_drives_runtime(client, db_session):
    """A FlowRuntime registered under flow_registry handles HTTP
    resolution end-to-end: scheduler completes the downstream after the
    task is resolved."""
    import flow_registry
    from flow_runtime import create_flow_state_db, EquationStore, FlowRuntime
    from flow_runtime import get_flow_state_db_path
    # Create a flow row so Flow.id exists in the main DB.
    rr = (await client.post("/api/flows", json={"name": "x-registry"})).json()
    flow_id = rr["id"]
    state_db = get_flow_state_db_path(flow_id)
    # Flow directory + state.db were created by the API.
    ...
    runtime = FlowRuntime(
        flow_id, state_db, flow_callable=r, evaluators=_mock_evaluators(),
    )
    runtime.build_initial_graph()
    await runtime.start()
    ...
    try:
        ...
        composite = f"{flow_id}:{rows[0]['task_id']}"
        r = await client.post(
...
@pytest.mark.asyncio
async def test_http_resolve_running_flow_inflates_missing_runtime(
    client, db_session, monkeypatch,
):
    """A running flow should not take the offline HITL path just because
    the in-memory registry is empty."""
    import flow_lifecycle
    import flow_registry
    from flow_runtime import get_flow_program_path, get_flow_state_db_path
    ...
    flow_id = rr["id"]
    get_flow_program_path(flow_id).write_text(
        '''from stimma.flow import flow, phase, hitl, code


@flow(name="inflate-running")
def prog():
    with phase("Run"):
        v = hitl.select(
            code("return ['here', 'there']", output_type="list[str]"),
...
'''
    ...
    try:
        ...
        # the per-flow runtime registry no longer has this flow loaded.
        await runtime.stop()
        flow_registry.unregister(flow_id)
        ...
        inflated = flow_registry.get_runtime(flow_id)
        ...
        active = flow_registry.get_runtime(flow_id)
...
async def test_http_resolve_select_offline_persists_hitl(client, db_session):
    ...
    r1 = (await client.post("/api/flows", json={"name": "x-offline"})).json()
    from flow_runtime import get_flow_state_db_path
    from flow_runtime.graph import Equation, EquationStatus as S, EquationType as ET
    state_db = get_flow_state_db_path(r1["id"])
    eq = Equation(
...
async def test_http_resolve_waiting_for_tool_skip_offline(client, db_session):
    r1 = (await client.post("/api/flows", json={"name": "x-waiting-tool"})).json()
    from flow_runtime import get_flow_state_db_path
    from flow_runtime.graph import Equation, EquationStatus as S, EquationType as ET
    state_db = get_flow_state_db_path(r1["id"])
    eq = Equation(
...
async def test_store_hit_restores_media_ids_and_duration(tmp_path):
    ...
    create_flow_state_db(state_db_1)
    create_flow_state_db(state_db_2)
    store = EquationStore(tmp_path / "store")
    store.initialize()
    ...
    runtime1 = FlowRuntime(
        301, state_db_1, flow_callable=r, evaluators=reg, store=store,
    )
    runtime1.build_initial_graph()
    await runtime1.start()
    await runtime1.wait_quiescent(timeout=3.0)
    ...
    runtime2 = FlowRuntime(
        302, state_db_2, flow_callable=r, evaluators=reg2, store=store,
    )
    runtime2.build_initial_graph()
    await runtime2.start()
    await runtime2.wait_quiescent(timeout=3.0)
