...
from flow_dsl.loader import build_graph_from_callable
from flow_dsl.primitives import (
    code,
    foreach,
    hitl,
    input as dsl_input,
...
class TestTupleShape:
    ...
    def test_zip_nodes_preserves_positional_source_order(self):
        ...
        from flow_dsl.context import BuildContext, activate_context, push_frame
        from flow_dsl.primitives import zip_nodes
        from flow_runtime.graph import EquationGraph
...
class TestCodeInputsInsideForeach:
    ...
    async def test_static_only_code_inputs_are_passed_to_sandbox(self):
        ...
        from flow_runtime.graph import Equation, EquationType
    ...
    async def test_engine_merges_static_inputs_when_no_dynamic(self):
        ...
        """
        from flow_runtime.engine import FlowRun
        from flow_runtime.graph import Equation, EquationGraph, EquationType
        ...
        # Minimal FlowRun stub: _resolve_dynamic_bindings only reads
        # self.graph, which we populated above.
        class _Stub:
            def __init__(self, g):
                self.graph = g

            _resolve_dynamic_bindings = FlowRun._resolve_dynamic_bindings
            _resolve_value = FlowRun._resolve_value
...
class TestLLMImagesValidation:
    ...
    def test_accepts_list_of_media_nodes(self):
        with patch("flow_dsl.primitives._tool_id_is_known", return_value=True):
            with patch("flow_dsl.primitives._get_tool_schema", return_value=None):
                @flow(
                    name="ok2",
                    inputs={"x": dsl_input(type="str")},
                    outputs={"out": dsl_output(type="str")},
