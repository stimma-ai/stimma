"""Unit tests for the ``fetch_media`` flow primitive + evaluator."""
...
from flow_dsl import fetch_media, flow as dsl_flow, web_search
...
from flow_runtime.evaluators import EvaluationRequest, EvaluatorError, TOOL_ERROR
from flow_runtime.production_evaluators import FetchMediaEvaluator, promote_url_media_value
...
class TestFetchMediaEvaluator:
    ...
    async def test_successful_image_calls_save_path(self):
        ...
        ev = FetchMediaEvaluator(fetcher=fake_fetch)
        with patch(
            "flow_runtime.production_evaluators._save_fetched_media", fake_save,
        ), patch(
            "flow_runtime.production_evaluators._tag_media_with_flow", fake_tag,
        ):
