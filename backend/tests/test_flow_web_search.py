"""Unit tests for the ``web_search`` flow primitive + evaluator."""

from __future__ import annotations

import pytest

from flow_dsl import flow as dsl_flow, web_search
from flow_dsl.primitives import phase as dsl_phase, input as dsl_input, output as dsl_output, code as dsl_code, foreach as dsl_foreach
...
from flow_runtime.production_evaluators import WebSearchEvaluator
...
class TestGraphBuild:
    ...
    def test_dynamic_query_records_input_dependency(self):
        ...
        assert len(web_eqs) == 1
        eq = web_eqs[0]
        # Dependency on the flow-input equation is registered.
