...
Covers the three production evaluators in ``recipe_runtime.production_evaluators``:

  - ``ToolCallEvaluator`` submits through the generation path and tags
    produced media with a recipe marker.
...
test that runs a ``tool() → code()`` recipe through a real ``RecipeRuntime``
with mocked providers. External services (generation queue, LLM, library
DB) are mocked in every unit test — the goal is to verify the recipe
...
from recipe_dsl import code as dsl_code, recipe as dsl_recipe, tool as dsl_tool
from recipe_dsl.primitives import phase as dsl_phase
...
from recipe_runtime.evaluators import (
    CODE_ERROR,
    LLM_ERROR,
    RESOURCE_ERROR,
    TOOL_ERROR,
...
# ---------------------------------------------------------------------------
# Integration — runtime runs a recipe end-to-end with mocked tool provider
# ---------------------------------------------------------------------------
...
async def test_runtime_runs_tool_then_code_via_production_registry(
    ...
    """A recipe with tool() + code() runs through RecipeRuntime wired with
    ...
    import recipe_dsl.primitives as primitives
    ...
    import recipe_runtime.production_evaluators as prod_evals
    ...
    async def fake_run(tool_id, inputs, *, seed, project_id=None):
        # The integration test cares that the recipe_id-bound inputs flow
    ...
    try:
        ...
        # Tagging fired once for the tool equation, with recipe_id=999 piped
        # through from the runtime config.
        assert len(tag_calls) == 1
        call = tag_calls[0]
        assert call["media_ids"] == [42]
        assert call["recipe_id"] == 999
...
# ---------------------------------------------------------------------------
# Integration — _tag_media_with_recipe writes to the real library DB
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_tag_media_with_recipe_writes_expected_metadata(
    test_app, db_session,
):
    """End-to-end write of the recipe tag onto a MediaItem row.
    ...
    from recipe_runtime.production_evaluators import _tag_media_with_recipe
    ...
    async with db_session() as session:
        img_path = Path(tempfile.gettempdir()) / "recipe_tag_test.png"
...
# ---------------------------------------------------------------------------
# RecipeRuntime threads project_id into the evaluator pipeline
# ---------------------------------------------------------------------------
...
async def test_runtime_threads_project_id_into_tool_evaluator(
    ...
    """``RecipeRuntime(project_id=...)`` must land in the EvaluationRequest so
    the tool evaluator can pass it down to the generation job, which is how
    recipe-generated media gets attached to the owning project.
    """
    _root, state_db, store = isolated_store_and_db
...
class TestCreateSetEvaluator:
    ...
    async def test_passes_project_id_to_assemble_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._open_session",
            _null_open_session,
        )
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._tag_media_with_recipe",
            _noop_tag,
        )
        ...
        req = EvaluationRequest(
            ...
            recipe_id=None,  # skips the explode-prior-assembly DB path
            project_id=900,
        )
...
class TestCreateDocumentEvaluator:
    ...
    async def test_passes_project_id_to_save_document_media(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._save_document_media",
            fake_save,
        )
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._tag_media_with_recipe",
...
class TestCreateImageEvaluator:
    ...
    async def test_invokes_callable_with_pil_inputs_and_saves(self, monkeypatch, tmp_path):
        ...
        from recipe_dsl.shapes import ListShape, Scalar
        ...
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._load_media_row",
            fake_row,
        )
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._save_pil_image_media",
            fake_save,
        )
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._tag_media_with_recipe",
    ...
    async def test_callable_exception_includes_line_numbered_traceback(
        ...
        """
        from recipe_runtime.production_evaluators import CreateImageEvaluator
    ...
    async def test_unknown_element_list_with_int_values_loads_as_media(
        ...
        from recipe_dsl.shapes import ListShape, UNKNOWN
        from recipe_runtime.graph import NodeRef
        from recipe_runtime.media_arg import Media
        from recipe_runtime.production_evaluators import CreateImageEvaluator
...
class TestCreateLayoutEvaluator:
    ...
    async def test_callable_receives_staged_filenames_and_saves(
        ...
        from recipe_dsl.shapes import Scalar
        from recipe_runtime.graph import NodeRef
        from recipe_runtime.production_evaluators import CreateLayoutEvaluator
        ...
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._stage_media_file_for_layout",
            fake_stage,
        )
        ...
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._save_layout_media",
            fake_save,
        )
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._tag_media_with_recipe",
    ...
    async def test_rescues_bare_int_ref_as_media_id(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._lookup_media_info",
            fake_lookup_info,
        )
        monkeypatch.setattr(
            "recipe_runtime.production_evaluators._save_layout_media",
...
class TestRasterizeLayoutEvaluator:
    ...
    async def test_rasterizes_layout_bundle_and_saves_png(self, monkeypatch):
        from PIL import Image as _Image
        from recipe_runtime.production_evaluators import RasterizeLayoutEvaluator
...
class TestFormatInfoValue:
    ...
    Python repr (the bug that put ``['a', 'b']`` on a recipe card).
    """
