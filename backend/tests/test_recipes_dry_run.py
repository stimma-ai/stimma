...
from recipe_runtime.dry_run import DryRunConfig, dry_run_recipe
...
@pytest.mark.asyncio
async def test_dry_run_success_samples_recipe_input_collection(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, input, output, foreach, hitl, phase, tool
...
async def test_dry_run_catches_deferred_tool_parameter_shape_error(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, hitl, tool, phase

@recipe(name="bad_tool_params", outputs={"out": output("list[media]")})
def bad_tool_params():
    with phase("Generate"):
        return hitl.approve(
            2,
...
''',
    ...
    with _with_fake_registry({
        ...
        report = await dry_run_recipe(recipe_id=4, program_path=program)
...
async def test_dry_run_executes_create_image_callable(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, create_image, phase

@recipe(name="bad_create_image", outputs={"out": output("media")})
...
''',
    ...
    report = await dry_run_recipe(recipe_id=5, program_path=program)
...
async def test_dry_run_does_not_sample_internal_approve_before_create_image(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, hitl, tool, create_image, phase
from PIL import Image

@recipe(name="six_panel", outputs={"out": output("media")})
def six_panel():
    with phase("Generate"):
        faces = hitl.approve(
            6,
...
async def test_dry_run_executes_create_layout_callable(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, create_layout, phase

@recipe(name="bad_layout", outputs={"out": output("media")})
def bad_layout():
    with phase("Compose"):
...
''',
    ...
    report = await dry_run_recipe(recipe_id=6, program_path=program)
...
async def test_dry_run_validates_create_grid_runtime_item_count(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, create_grid, code, phase

@recipe(name="bad_grid", outputs={"out": output("media")})
def bad_grid():
    with phase("Compose"):
...
''',
    ...
    report = await dry_run_recipe(recipe_id=7, program_path=program)
...
async def test_dry_run_catches_deferred_callback_noderef_misuse(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, foreach, code, phase

@recipe(name="bad_callback", outputs={"out": output("list[int]")})
def bad_callback():
    with phase("Build"):
...
''',
    ...
    report = await dry_run_recipe(recipe_id=8, program_path=program)
...
@recipe(name="bad_callback_signature", outputs={"out": output("list[int]")})
...
''',
    ...
    report = await dry_run_recipe(recipe_id=10, program_path=program)
...
async def test_dry_run_catches_runtime_code_shape_error(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, llm, code, phase

@recipe(name="bad", outputs={"items": output("list[str]")})
def bad():
    with phase("Check"):
...
''',
    ...
    report = await dry_run_recipe(recipe_id=2, program_path=program)
...
async def test_dry_run_llm_response_format_flows_through_code_and_hitl(tmp_path):
    program = _write_program(
        tmp_path,
        '''
from stimma.recipe import recipe, output, llm, code, hitl, phase

@recipe(name="structured", outputs={"item": output("str")})
def structured():
    with phase("Check"):
        data = llm(
            "return json",
...
''',
    ...
    report = await dry_run_recipe(recipe_id=3, program_path=program)
