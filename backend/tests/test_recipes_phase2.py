"""Phase 2 tests for the Recipes feature — FRP runtime core.

Covers the Phase 2 exit gate from docs/RECIPES_DEV_PLAN.md §Phase 2:

1. Hand-written recipe with foreach + HITL + parallel branches runs end-to-end.
...
These use real per-recipe SQLite files + real equation store but mock the
...
from recipe_runtime.graph import can_transition
from recipe_runtime.graph_builder import build_graph_from_callable
...
class TestEquationKeys:
    ...
    def test_at_prefix_rejected_in_function_name(self):
        with pytest.raises(EquationKeyError):
            validate_function_name("@sub_recipe")
...
def _cocktail_recipe(cocktail_names, style):
    def research(name):
...
class TestGraphBuild:
    def test_initial_graph_structure(self):
        ...
        keys = graph.keys()
        assert "_cocktail_recipe/recipe_input$0" in keys
        assert any(k.startswith("_cocktail_recipe/foreach$0") for k in keys)
        assert any(k.startswith("_cocktail_recipe/foreach$1") for k in keys)
    ...
    def test_graph_build_error_surfaces_traceback(self):
        ...
        from recipe_runtime import GraphBuildError
...
async def test_linear_chain_executes_in_order(isolated_store_and_db):
    ...
    @recipe(name="linear", inputs={"name": dsl_input(type="str")}, outputs={"out": dsl_output(type="str")})
    def linear_recipe(name):
        with phase("Compute"):
    ...
    runtime = RecipeRuntime(
        1, db_path, recipe_callable=linear_recipe, inputs={"name": "x"},
...
@pytest.mark.asyncio
async def test_missing_recipe_input_blocks_downstream_equations(isolated_store_and_db):
    db_path, store = isolated_store_and_db

    @recipe(name="needs_input", inputs={"name": dsl_input(type="str")}, outputs={"out": dsl_output(type="str")})
    def needs_input(name):
        with phase("Compute"):
            return code("return name.upper()", inputs={"name": name})

    runtime = RecipeRuntime(
        101, db_path, recipe_callable=needs_input, inputs={},
    ...
    assert by_type[EquationType.RECIPE_INPUT].status == EquationStatus.PENDING
    assert by_type[EquationType.CODE].status == EquationStatus.PENDING


@pytest.mark.asyncio
async def test_recipe_input_defaults_are_runtime_values(isolated_store_and_db):
    db_path, store = isolated_store_and_db
    ...
    runtime = RecipeRuntime(
        104, db_path, recipe_callable=defaulted_input, inputs={},
        evaluators=_make_mock_evaluators(), store=store,
    )
    runtime.build_initial_graph()
    ...
    recipe_input = next(
        ...
        if eq.equation_type == EquationType.RECIPE_INPUT
    )
    assert recipe_input.status == EquationStatus.COMPLETED
    assert recipe_input.result == "jazz singer"
    assert recipe_input.is_waiting_for_recipe_input() is False


@pytest.mark.asyncio
async def test_optional_recipe_input_completes_with_none_and_unblocks_downstream(
    isolated_store_and_db,
):
    # Regression: before the ``optional=True`` flag landed, a recipe input
    # the user hadn't filled in sat at PENDING forever (``is_waiting_for_
    # recipe_input`` guarded any None value), wedging every downstream
    ...
    runtime = RecipeRuntime(
        103, db_path, recipe_callable=optional_input,
        inputs={"name": "widget"},
        evaluators=_make_mock_evaluators(), store=store,
    )
    runtime.build_initial_graph()
    ...
    assert by_name["note"].is_waiting_for_recipe_input() is False


@pytest.mark.asyncio
async def test_missing_list_recipe_input_still_builds_as_collection(isolated_store_and_db):
    db_path, store = isolated_store_and_db

    def add_bang(name):
        return code("return name + '!'", inputs={"name": name})

    @recipe(name="list_input", inputs={"names": dsl_input(type="list[str]")}, outputs={"out": dsl_output(type="list[str]")})
    def list_input(names):
        with phase("Compute"):
            return foreach(names, add_bang)

    runtime = RecipeRuntime(
        102, db_path, recipe_callable=list_input, inputs={},
        evaluators=_make_mock_evaluators(), store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    ...
    assert recipe_input.definition["is_collection"] is True
    assert recipe_input.status == EquationStatus.PENDING
...
async def test_parallel_branches_run_concurrently(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        2, db_path, recipe_callable=parallel_recipe, evaluators=reg, store=store,
    )
...
async def test_foreach_expansion(isolated_store_and_db):
    ...
    @recipe(inputs={"names": dsl_input(type="list[str]")})
    def simple(names):
        def one(name):
    ...
    reg = _make_mock_evaluators()
    runtime = RecipeRuntime(
        3, db_path, recipe_callable=simple, inputs={"names": ["Alice", "Bob", "Carol"]},
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_nested_foreach(isolated_store_and_db):
    ...
    @recipe(inputs={"groups": dsl_input(type="list[dict]")})
    def nested(groups):
    ...
    reg = _make_mock_evaluators()
    runtime = RecipeRuntime(
        4, db_path, recipe_callable=nested,
...
async def test_chained_foreach_expands_downstream_before_upstream_completes(
    ...
    reg = _make_mock_evaluators()
    runtime = RecipeRuntime(
        5001, db_path, recipe_callable=pipeline,
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_chained_foreach_pipelines_per_item(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        5003, db_path, recipe_callable=pipeline,
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_chained_foreach_falls_back_when_callback_inspects_item(
    ...
    recipe must still complete correctly — just without pipelining.
    """
    db_path, store = isolated_store_and_db
    ...
    runtime = RecipeRuntime(
        5002, db_path, recipe_callable=pipeline,
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_chained_foreach_preserves_per_iteration_upstream_deps(
    ...
    runtime = RecipeRuntime(
        5004, db_path, recipe_callable=pipeline,
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_invalidate_rebuilds_nested_foreach_extra_kwarg_subgraph(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        4001,
        db_path,
        recipe_callable=nested_extra_kwarg,
        evaluators=reg,
        store=store,
    )
    runtime.build_initial_graph()
...
def test_info_inherits_phase_dependencies():
    ...
    Without this, an info card whose only explicit inputs are recipe inputs
    ...
    """
    from recipe_dsl.primitives import info

    @recipe(inputs={"x": dsl_input(type="str")})
    def r(x):
        with phase("Outer"):
    ...
    # not just the recipe-input it explicitly references.
...
def test_info_depends_on_foreach_wrapper_in_same_phase():
    ...
    """
    from recipe_dsl.primitives import code as dsl_code
    from recipe_dsl.primitives import foreach, info
...
def test_info_title_and_subtitle_stored_in_definition():
    """Title and subtitle land on the equation definition so routes/recipes.py
...
async def test_foreach_iteration_callback_error_creates_task(isolated_store_and_db):
    ...
    @recipe(inputs={"items": dsl_input(type="list[str]")})
    def r(items):
    ...
    reg = _make_mock_evaluators()
    runtime = RecipeRuntime(
        9901, db_path, recipe_callable=r,
        inputs={"items": ["a", "b", "c"]},
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
...
def test_completed_deferred_foreach_is_not_reexpanded(isolated_store_and_db):
    ...
    run = RecipeRun(
        runtime.graph,
        RecipeRunConfig(recipe_id=9902, state_db_path=db_path),
...
async def test_hitl_blocks_and_unblocks_on_resolve(isolated_store_and_db):
    ...
    reg = _make_mock_evaluators()
    runtime = RecipeRuntime(
        5, db_path, recipe_callable=with_hitl, inputs={"names": ["A", "B"]},
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_invalidation_cascades_and_rebumps_attempt(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        6, db_path, recipe_callable=chain, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
...
async def test_store_hits_prevent_redundant_computation(isolated_store_and_db):
    ...
    rt1 = RecipeRuntime(
        10, db_path, recipe_callable=r, inputs={"names": ["Alice", "Bob"]},
    ...
    create_recipe_state_db(db_path2)

    counter2: dict[str, int] = {}
    reg2 = _make_mock_evaluators(tool_call_counter=counter2)
    rt2 = RecipeRuntime(
        11, db_path2, recipe_callable=r, inputs={"names": ["Alice", "Bob"]},
...
async def test_pause_resume_preserves_state(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        20, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_invalidate_computing_equation_reruns_immediately(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        21, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    try:
...
async def test_invalidate_restarts_dead_scheduler_task(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        22, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    try:
...
async def test_transient_error_retries_up_to_3(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        30, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
...
async def test_tool_error_fails_immediately(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        31, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
...
async def test_llm_error_retries_once(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        32, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
...
@pytest.mark.asyncio
async def test_resource_exhaustion_pauses_recipe(isolated_store_and_db):
    db_path, store = isolated_store_and_db
    ...
    runtime = RecipeRuntime(
        33, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_loop_failure_isolation(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        34, db_path, recipe_callable=r, inputs={"names": ["A", "B", "C"]},
        evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
...
async def test_concurrency_cap_holds_under_load(isolated_store_and_db):
    ...
    # 20 independent equations — all directly under the recipe root.
    @recipe()
    def wide():
        with phase("Run"):
    ...
    runtime = RecipeRuntime(
        40, db_path, recipe_callable=wide, evaluators=reg, store=store,
...
async def test_reload_program_preserves_unchanged(isolated_store_and_db):
    ...
    reg = _make_mock_evaluators(tool_call_counter=counter)
    runtime = RecipeRuntime(
        50, db_path, recipe_callable=first, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
    ...
    runtime.recipe_callable = second
...
async def test_reload_program_same_name_preserves_unchanged(isolated_store_and_db):
    ...
    reg = _make_mock_evaluators(tool_call_counter=counter)
    runtime = RecipeRuntime(
        51, db_path, recipe_callable=_make(1), evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
    ...
    runtime.recipe_callable = _make(2)
    diff = runtime.reload_program()
...
async def test_app_restart_recovery_resets_computing(isolated_store_and_db):
    ...
    from recipe_runtime.graph_db import reset_computing_to_pending
...
def test_runtime_recovery_resets_orphaned_in_memory_computing(isolated_store_and_db):
    ...
    @recipe(name="orphaned", outputs={"out": dsl_output(type="str")})
    def orphaned():
        with phase("Choose"):
    ...
    run = RecipeRun(
        graph,
        RecipeRunConfig(recipe_id=1, state_db_path=db_path),
        evaluators=_make_mock_evaluators(),
        store=store,
    )
...
async def test_switch_selects_value_without_dynamic_subgraph(isolated_store_and_db):
    ...
    @recipe(inputs={"genre": dsl_input(type="str", default="jazz")})
    def r(genre):
        with phase("Render"):
            prompt = switch(
                genre,
    ...
    runtime = RecipeRuntime(
        60, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
...
async def test_filter_and_take_route_collections_statically(isolated_store_and_db):
    ...
    @recipe(inputs={"items": dsl_input(type="list[json]")})
    def r(items):
        with phase("Approve"):
            approved = filter(
                items,
...
async def test_when_and_gate_route_values_statically(isolated_store_and_db):
    ...
    runtime = RecipeRuntime(
        62, db_path, recipe_callable=r, evaluators=reg, store=store,
    )
    runtime.build_initial_graph()
    await runtime.start()
    await runtime.wait_quiescent(timeout=5.0)
