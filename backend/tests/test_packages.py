...
def test_builtin_and_stimpack_recipes_are_listed():
    ids = {r.id for r in list_recipes()}
    assert {"tiles", "logo", "key-art-crops"} <= ids
    assert next(r for r in list_recipes() if r.id == "tiles").source == "test-tiles"
...
async def test_builtin_recipes_are_deterministic(tmp_path):
    ...
    assert await check_determinism(get_recipe("tiles"), {"master": _resolved("master", tmp_path / "master.png")}, {"background": "#FFFFFF", "app_name": "Acme", "platforms": ["ios"]}) == []
...
async def test_builder_saves_package_with_run_and_asset(db_session, tmp_path):
    ...
    async with db_session() as session:
        ...
        async with PackageBuilder(session, profile_id="default", title="Acme icons") as builder:
            ...
            rid = await builder.run("tiles", {"master": mid}, {"background": "#FFFFFF", "app_name": "Acme", "platforms": ["ios"]})
        ...
        assert manifest["runs"][0]["id"] == rid and manifest["runs"][0]["recipe"]["id"] == "tiles"
...
async def test_stale_after_master_revision_and_rebuild(db_session, tmp_path):
    ...
    async with db_session() as session:
        ...
        async with PackageBuilder(session, profile_id="default", title="Acme icons") as builder:
            await builder.run("tiles", {"master": await builder.add_member(master.id, role="master")}, {"background": "#FFFFFF", "app_name": "Acme", "platforms": ["web"]})
...
async def test_exports(db_session, tmp_path):
    ...
    async with db_session() as session:
        ...
        async with PackageBuilder(session, profile_id="default", title="Export me") as builder:
            await builder.run("tiles", {"master": await builder.add_member(master.id)}, {"background": "#FFFFFF", "app_name": "Acme", "platforms": ["web"]})
        ...
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
            ...
            assert "export-me/tiles/web/tile-256.png" in names
            assert "export-me/tiles.zip" in names
            assert "export-me/extras/brief.txt" in names
            assert not any("/_stimma/" in n for n in names)
            with zipfile.ZipFile(io.BytesIO(zf.read("export-me/tiles.zip"))) as inner:
                assert "web/tile-256.png" in inner.namelist()
...
async def test_package_routes(client, db_session, tmp_path):
    ...
    assert any(r["id"] == "tiles" and any(i["name"] == "master" for i in r["inputs"]) for r in recipes)

    created = await client.post("/api/packages", json={
        "title": "Route icons", "media_ids": [master_id], "recipe": "tiles",
    ...
    assert info["status"]["stale"] is False and info["status"]["runs"][0]["recipe"]["id"] == "tiles"
    ...
    readme = await client.get(f"/api/media/{media_id}/package-file/tiles/README.txt")
    assert readme.status_code == 200 and "Tiles for Acme" in readme.text
    run_zip = await client.get(f"/api/media/{media_id}/package-file/tiles.zip")
...
async def test_draft_inspection_and_workspace_preview_before_save(db_session, tmp_path):
    ...
    async with db_session() as session:
        ...
        try:
            ...
            await pkg.run("tiles", {"master": member}, {"platforms": ["ios"], "background": "#FFFFFF", "app_name": "Acme"})
            await pkg.run("tiles", {"master": member}, {"platforms": ["android", "web"], "background": "#FFFFFF", "app_name": "Acme"})
            manifest = await pkg.manifest()
            assert [r["root"] for r in manifest["runs"]] == ["tiles/", "tiles-2/"]
            assert manifest["cover"]["kind"] == "auto"
            ...
            with pytest.raises(CoverError, match="does not resolve"):
                pkg.set_cover('<stimma-media ref="r1/ios/tile-256.png"></stimma-media>')
            pkg.set_cover('<stimma-media ref="tiles/ios/tile-256.png"></stimma-media><stimma-files ref="r2"></stimma-files>')
            authored = workspace / await pkg.preview()
            assert pkg.media_id is None
            assert sdk._session_media_ids == []
            assert "tiles/ios/tile-256.png" in (authored / "index.html").read_text()
...
@pytest.mark.parametrize('source', [
    None,
    '<div class="sp-page"><h1>Collection</h1><div>Contents</div></div>',
    '<main><h1>Collection</h1></main>',
    '<div class="sp-page"><p class="sp-footer">Made with Stimma</p></div>',
])
def test_every_cover_has_one_kit_signature(source):
    from packages.cover import render_cover_document
    from packages.kit import footer

    document, problems = render_cover_document({'title': 'Collection'}, authored_html=source)
    assert problems == []
    assert document.count('<p class="sp-footer">') == 1
