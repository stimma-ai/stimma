...
from flow_dsl.loader import load_program_with_error_classification
...
class TestNodeInspectionGuards:
    def _build_flow_that(self, body):
        """Helper — wrap a no-input flow body so we have an active build context."""
        @flow(name="guard")
        def r():
            return body()
        return r
    ...
    def test_resolved_value_works_in_foreach_callback(self):
        ...
        @flow(name="r", inputs={"names": dsl_input("list[str]")})
        def r(names):
        ...
        # the wrapper and the flow_input; sub-graph iterations expand at
...
class TestPrimitiveShapes:
    ...
    def test_routing_primitive_display_names(self):
        from routes.flows import _display_name_for_equation
...
class TestPhases:
    ...
    def test_phase_order_does_not_change_keys(self):
        ...
        # across parses (the flow's __name__ is the equation-key root).
        src_before = """
from stimma.flow import flow, phase, llm
...
class TestForeachKeyDerivation:
    def test_scalar_list_keys_by_value(self):
        ...
        # The flow_input equation knows the keys.
        flow_inputs = [e for e in g.all_equations() if e.equation_type == EquationType.FLOW_INPUT]
        assert len(flow_inputs) == 1
        assert flow_inputs[0].definition["element_kind"] == "scalar"

    def test_json_list_canonical_hash(self):
        @flow(name="r", inputs={"items": dsl_input("list[json]")})
        def r(items):
        ...
        flow_inputs = [e for e in g.all_equations() if e.equation_type == EquationType.FLOW_INPUT]
        assert flow_inputs[0].definition["element_kind"] == "json"
...
class TestKeyStability:
    def test_rebuild_identical_graph(self):
        src = """
from stimma.flow import flow, input, output, foreach, tool, llm, hitl, phase
...
"""
    ...
    def test_program_edit_preserves_unrelated_keys(self):
        ...
        src_a = """
from stimma.flow import flow, input, output, llm, phase

@flow(name="x", inputs={"n": input("str")}, outputs={"r": output("str")})
def r(n):
    with phase("a"):
        return llm(prompt="x")
"""
        src_b = """
from stimma.flow import flow, input, output, llm, code, phase
...
class TestLoaderErrors:
    ...
    def test_name_error(self):
        src = """
from stimma.flow import flow, output

@flow(name="x", outputs={"r": output("str")})
def r():
...
"""
    ...
    def test_dsl_misuse_surfaces_category(self):
        ...
        src = """
from stimma.flow import flow, output, llm, hitl, phase
...
"""
    ...
    def test_missing_flow_decorator(self):
        src = """
def not_a_flow():
    return None
"""
    ...
    def test_multiple_flow_decorators_rejected(self):
        src = """
from stimma.flow import flow, output, llm, phase

@flow(name="a", outputs={"r": output("str")})
def a():
    with phase("p"):
        return llm(prompt="a")

@flow(name="b", outputs={"r": output("str")})
def b():
    with phase("p"):
        return llm(prompt="b")
"""
        with pytest.raises(ProgramLoadError) as exc:
            build_graph_from_source(src)
        assert exc.value.category == "dsl"
        assert "multiple @flow" in str(exc.value)
...
# Minimal no-op evaluator so FlowRuntime.build_initial_graph completes when
...
class TestRuntimeUpdateFlow:
    @pytest.fixture
    def flow_setup(self, tmp_path):
        state_db = tmp_path / "state.db"
        create_flow_state_db(state_db)
        return tmp_path, state_db

    def test_successful_edit_preserves_unchanged_keys(self, flow_setup):
        flow_dir, state_db = flow_setup
        src_a = """
from stimma.flow import flow, input, output, llm, phase
...
def r(n):
    with phase("a"):
        return llm(prompt="x")
"""
        _write_program(flow_dir, src_a)
        runtime = FlowRuntime(
            flow_id=1,
            state_db_path=state_db,
            program_path=flow_dir / "program.py",
            inputs={"n": "foo"},
...
"""
    ...
    def test_build_initial_graph_completes_provided_flow_input(self, flow_setup):
        """A populated flow input should not sit in pending before start()."""
        flow_dir, state_db = flow_setup
        src = """
from stimma.flow import flow, input, output, code, phase
...
"""
    ...
    def test_build_initial_graph_prunes_orphan_equations(self, flow_setup):
        """Equations from a prior flow version must not survive startup.
...
"""
    ...
    def test_broken_edit_keeps_previous_graph(self, flow_setup):
        flow_dir, state_db = flow_setup
        src_good = """
from stimma.flow import flow, input, output, llm, phase
...
"""
        _write_program(flow_dir, src_good)
        runtime = FlowRuntime(
            flow_id=2,
            state_db_path=state_db,
            program_path=flow_dir / "program.py",
        ...
        _write_program(flow_dir, "def broken(\n")  # syntax error
    ...
    def test_rollback_restores_good_program(self, flow_setup):
        flow_dir, state_db = flow_setup
        src_good = """
from stimma.flow import flow, input, output, llm, phase
...
"""
        ...
        assert (flow_dir / "program.py").read_text() == src_good
        assert runtime.last_load_error is None

    def test_version_list_tracks_each_edit(self, flow_setup):
        flow_dir, state_db = flow_setup
        v1 = """
from stimma.flow import flow, input, output, llm, phase
...
"""
        ...
        v2 = v1.replace("v1", "v2")
        _write_program(flow_dir, v2)
        runtime.reload_program()

        v3 = v1.replace("v1", "v3")
        _write_program(flow_dir, v3)
        runtime.reload_program()
    ...
    def test_scalar_input_change_invalidates_downstream(self, flow_setup):
        """Changing a scalar flow input must invalidate cached downstream
        results. Without the fix, _def_hash ignored the "value" field of
        scalar flow_input equations so the diff marked them unchanged,
...
"""
        ...
        # The flow_input equation must be detected as changed.
    ...
    def test_foreach_callback_body_edit_invalidates_wrapper(self, flow_setup):
        ...
        flow_dir, state_db = flow_setup
        src_fox = """
from stimma.flow import flow, input, output, phase, foreach, tool, hitl

@flow(name="x", inputs={"n": input("str")}, outputs={"r": output("media")})
def r(n):
    with phase("gen"):
...
"""
        _write_program(flow_dir, src_fox)
        runtime = FlowRuntime(
            flow_id=200,
            state_db_path=state_db,
            program_path=flow_dir / "program.py",
        ...
        _write_program(flow_dir, src_duck)
...
"""
    ...
    def test_cocktail_posters_structure(self):
        ...
        src = """
from stimma.flow import flow, input, output, phase, foreach, tool, llm, hitl, zip_nodes
...
"""
    ...
    def test_program_with_switch_builds_static_shape(self):
        ...
        src = """
from stimma.flow import flow, input, output, foreach, tool, llm, code, switch
...
async def test_count_one_approve_blocks_downstream_until_slot_approved(isolated_store_and_db):
    ...
    """
    flow_dir, state_db, store = isolated_store_and_db

    src = """
from stimma.flow import flow, output, phase, tool, hitl

@flow(name="headshot_refs", outputs={"ref": output("media")})
def r():
    with phase("Headshot"):
...
"""
    (flow_dir / "program.py").write_text(src)
...
async def test_approve_each_over_input_list_creates_one_task_per_item(isolated_store_and_db):
    ...
    src = """
from stimma.flow import flow, input, output, phase, tool, hitl
...
async def test_approve_reject_regenerates_full_slot_scope(isolated_store_and_db):
    ...
    src = """
from stimma.flow import flow, output, phase, tool, llm, code, hitl

@flow(name="slot_scope", outputs={"image": output("media")})
def r():
    with phase("Generate"):
...
"""
    (flow_dir / "program.py").write_text(v1)
...
"""
    (flow_dir / "program.py").write_text(v2)
...
# =============================================================================
# Flow agent — prompt sanity
# =============================================================================


class TestFlowAgentPrompt:
    def test_prompt_covers_dsl_rules(self):
        from agent.v2.flow_prompt import FLOW_SYSTEM_PROMPT

        # Spot-check that the positive principles the experiment report
        # flagged are all in the prompt.
        assert "foreach" in FLOW_SYSTEM_PROMPT
        ...
        assert "hitl.select(candidates, *, instructions, count=1)" in FLOW_SYSTEM_PROMPT
        assert "hitl.approve_one(generate, *, instructions, **kwargs)" in FLOW_SYSTEM_PROMPT
        assert "hitl.approve_each(items, generate, *, instructions, **kwargs)" in FLOW_SYSTEM_PROMPT
        assert "hitl.approve(count, generate, *, instructions, **kwargs)" in FLOW_SYSTEM_PROMPT
        assert "Do **not**" in FLOW_SYSTEM_PROMPT
        assert "gate(value, condition=...)" in FLOW_SYSTEM_PROMPT
        assert "gate(approved, image" in FLOW_SYSTEM_PROMPT
        assert "callback body IS the regen" in FLOW_SYSTEM_PROMPT
        assert "Reject to regenerate this slot" in FLOW_SYSTEM_PROMPT
        assert "Node" in FLOW_SYSTEM_PROMPT  # calls out the output-type mistake
        assert "code(" in FLOW_SYSTEM_PROMPT  # list extraction pattern
        assert "what should change when the user clicks" in FLOW_SYSTEM_PROMPT
        assert "Put every step that should change inside" in FLOW_SYSTEM_PROMPT
        assert "Use approval kwargs for context" in FLOW_SYSTEM_PROMPT
        assert "hitl" in FLOW_SYSTEM_PROMPT
        assert "title=\"Mood Reading\"" in FLOW_SYSTEM_PROMPT
        assert "title=\"Selection Confirmed\"" in FLOW_SYSTEM_PROMPT
        ...
        assert "Do not write code-change summaries" in FLOW_SYSTEM_PROMPT

    def test_flow_update_tool_registered(self):
        ...
        t = get_tool("flow_update")
        assert t is not None
        param_names = {p.name for p in t.parameters}
        # program.py owns input/output schemas; flow_update only touches
