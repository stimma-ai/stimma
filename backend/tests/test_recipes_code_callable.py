...
the double-escape class of f-string bugs. See RECIPES_DSL.md §3, §6.
"""
...
from recipe_runtime.graph import EquationType
from recipe_runtime.store_key import definition_hash_for_code
...
def test_lambda_captures_canonical_source():
    @recipe(name="r", outputs={"out": dsl_output("str")})
    def r():
        with phase("P"):
...
def test_string_form_still_accepted():
    """Back-compat shim — existing agent-authored recipes still work."""
    @recipe(name="r", outputs={"out": dsl_output("str")})
    def r():
        with phase("P"):
            greeting = llm("x")
...
def test_whitespace_variants_hash_equal():
    @recipe(name="a", outputs={"o": dsl_output("text")})
    def a():
        with phase("P"):
            return code(lambda n: f"x {n}", inputs={"n": "y"}, output_type="text")

    @recipe(name="b", outputs={"o": dsl_output("text")})
    def b():
        with phase("P"):
...
def test_closure_over_noderef_rejected():
    ...
    @recipe(name="r", outputs={"o": dsl_output("text")})
    def r():
        with phase("P"):
            greeting = llm("x")
...
# ----- Recipe return/output contract ---------------------------------------


def test_single_declared_output_allows_bare_return_and_tracks_name():
    @recipe(name="r", outputs={"final_photos": dsl_output("list[media]")})
    def r():
        with phase("P"):
            return llm("x")
...
async def test_callable_can_use_allowed_module_globals_without_import():
    """Recipe callables keep their module globals until evaluation.
    ...
    src = """
from stimma.recipe import recipe, output, code, phase

@recipe(name="r", outputs={"out": output("json")})
def r():
    with phase("Parse"):
