...
graph builder before executing the recipe function; foreach callbacks push
a fresh frame so their nested DSL calls get keys
relative to the callback.

Kept in its own module so both recipe_runtime internals and the user-facing
...
if TYPE_CHECKING:
    ...
    # recipe_runtime/__init__.py re-exports the DSL shim.
    from recipe_runtime.graph import EquationGraph  # pragma: no cover
...
class BuildFrame:
    ...
    def equations_in_phase(self, phase_stack: list[str]) -> list[str]:
        ...
        cards don't implicitly depend on the whole recipe.
        """
        if not phase_stack:
            return []
...
class BuildContext:
    ...
    # recipe_runtime.graph here — see the TYPE_CHECKING guard above.
    graph: Any
...
_CURRENT_CONTEXT: contextvars.ContextVar[Optional[BuildContext]] = (
    contextvars.ContextVar("stimma_recipe_build_context", default=None)
)
...
def current_context() -> BuildContext:
    ...
    if ctx is None:
        raise RuntimeError(
            "no active BuildContext — recipe DSL called outside graph construction"
        )
    return ctx
