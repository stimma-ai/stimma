"""Stimma recipe DSL — the Python surface the recipe agent writes against.

See docs/RECIPES_DSL.md for the full design. In short:

- ``@recipe(name=..., inputs=..., outputs=...)`` decorates the top-level
  recipe function. The function body builds an equation graph.
- ``input()`` / ``output()`` declare the recipe's interface.
...
- ``foreach(items, callback, **extra)`` registers a loop. Iteration keys
  are derived by the runtime (RECIPES_EQUATION_KEYS §6); no ``key=``.
