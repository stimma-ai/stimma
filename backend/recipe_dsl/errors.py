...
class DSLError(Exception):
    """Base for any error surfaced from the recipe DSL layer.
...
class NodeUsageError(DSLError):
    ...
    as the leading phrase, which matches the language in RECIPES_DSL.md §8.
    """
...
class ProgramLoadError(DSLError):
    ...
    so the UI can show the right action (edit recipe vs. retry).
    """
