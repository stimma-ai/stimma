"""Program version tracking for recipe rollback.

Each recipe directory gets a ``versions/`` subdirectory. Every time the
...
deferred — the expected version count per recipe is small (tens, not
thousands).
"""
...
class ProgramVersionStore:
    """File-backed history of program.py for a single recipe.
    ...
    def __init__(self, recipe_dir: Path) -> None:
        ...
        self.versions_dir = self.recipe_dir / self.VERSIONS_DIR
    ...
    def program_path(self) -> Path:
        """Absolute path of the recipe's current program.py."""
        return self.recipe_dir / "program.py"
...
def get_version_store(recipe_dir: Path) -> ProgramVersionStore:
    return ProgramVersionStore(recipe_dir)
