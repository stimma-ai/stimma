"""DSL primitives — @flow, phase, foreach, tool, llm, code, hitl.*, zip_nodes.

This is the Python surface the flow agent imports. Each primitive
...
from flow_runtime.graph import (
    DeferredExpansion,
    Equation,
    EquationType,
    NodeRef,
...
# ----- @flow decorator + input/output specs -------------------------------
...
class InputSpec:
    ...
    # Render hint for the flow UI. Only meaningful for ``type="str"``:
    ...
    optional: bool = False
    # Human-readable label shown in the flow UI (steps/graph/input form)
    ...
    display_name: str = ""
    # Optional UI hints for the flow input form. Keep this small and
...
class FlowDecorated:
    """Wraps the user's flow function with its schema metadata."""
...
def flow(
    *,
    ...
    def wrap(fn: Callable[..., Any]) -> FlowDecorated:
        return FlowDecorated(
            fn,
...
def tool(tool_id: str, *, task_type: Any = _TASK_TYPE_MISSING, **params) -> NodeRef:
    ...
    call is doing so the flow row can render as the action ("Generate
    ...
    # the flow at parse time. The engine treats an unknown tool at
    ...
    if isinstance(images, NodeRef):
        ...
        raise DSLMisuseError(
            ...
            suggestion=(
                ...
                "output, passed in via a flow input of type='media', or a "
            ...
            if not isinstance(item, NodeRef):
                raise DSLMisuseError(
                    ...
                    suggestion=(
                        ...
                        "results or flow inputs of type='media'. Plain strings, "
...
def info(
    ...
    *,
    ...
    # phase — "here's what the flow just did". Make it implicitly depend
    ...
    # Without this, an info whose only explicit inputs are flow inputs
            ...
            if not isinstance(it, NodeRef):
                raise DSLMisuseError(
                    ...
                    suggestion=(
                        ...
                        "tool() calls or media flow inputs. Plain ints, "
...
def create_image(
    ...
    *,
    ...
    """Render a PIL image inside the flow sandbox and save it to the library.

    This is the primitive for flow steps that compose a single image from
...
def create_layout(
    ...
    *,
    ...
    Use this when the flow's output is a designed composition — product
...
def rasterize_layout(
    ...
    *,
    ...
    Use sparingly — layouts are usually the terminal output of a flow. Reach
...
def web_search(
    ...
    *,
    ...
    string or a single NodeRef (e.g. a flow input or ``code()`` result).
    ...
    if not query_template.strip() and not query_dynamic:
        raise DSLMisuseError(
            ...
            suggestion=(
                ...
                "flow input/code() result through (web_search(input.query))."
            ),
        )
...
def gate(
    ...
    *,
    ...
    """Alias for ``when``; reads better for approval/filtering flows."""
...
def filter(  # noqa: A001 - intentionally part of the flow DSL surface
    items: Any,
    predicate: Callable[[Any], bool],
    *,
    output_type: str = "list[json]",
...
def foreach(items: Any, callback: Callable[..., Any], **extra_kwargs) -> NodeRef:
    ...
    if isinstance(items, NodeRef):
        ...
        raise DSLMisuseError(
            ...
            suggestion=(
                "Pass a flow input list, a node returned by foreach or "
...
def _infer_foreach_element_shape(
    ...
    """
    from flow_runtime.graph import EquationGraph as _Graph
...
def _validate_approve_generator(
    ...
    build-time error so the flow can't silently behave as a no-op
    reject.
...
def foreach_range(
    ...
    **extra_kwargs: Any,
    ...
    Use when the iteration count is itself a value (a flow int input, a
    ...
    if isinstance(n, bool) or not isinstance(n, (int, NodeRef)):
        raise DSLMisuseError(
            ...
            f"{type(n).__name__}",
            suggestion=(
                "Pass a flow input of type='int', a code() node with "
...
def _register_literal_collection(ctx: BuildContext, values: Iterable[Any]) -> NodeRef:
    ...
    eq = Equation(
        key=key,
        equation_type=EquationType.FLOW_INPUT,
        definition={
...
def lift_flow_input(
    ...
    *,
    ...
    """Materialize a flow input as a FLOW_INPUT equation node.
    ...
    inputs no longer "pass through" as raw Python values; the flow
    ...
    structure.
    """
    pos = ctx.current_frame.next_positional("flow_input")
    key = make_nested_dsl_key(ctx.current_frame.parent_key, "flow_input", pos)
