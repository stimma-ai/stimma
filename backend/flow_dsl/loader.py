...
Errors are classified so the flow UI and the agent can act appropriately:
...
from flow_runtime.graph import EquationGraph, EquationType, NodeRef
...
# the import shape flows declare at the top of ``program.py``:
#
#     from stimma.flow import flow, foreach, tool, llm, code, hitl, ...
...
def _ensure_stimma_flow_module_registered() -> tuple[types.ModuleType, types.ModuleType]:
    """Create a virtual ``stimma.flow`` module that re-exports the DSL.

    This lets flow programs use the canonical import shape the docs and
    system prompt show:

        from stimma.flow import flow, foreach, tool, ...

    We install it in ``sys.modules`` so the first ``import stimma.flow``
    hits this virtual module instead of failing. Idempotent. Returns the
    ``(stimma, stimma.flow)`` module pair so callers can expose them to
    ...
    flow_module = sys.modules.get("stimma.flow")
    if flow_module is None:
        flow_module = types.ModuleType("stimma.flow")
        sys.modules["stimma.flow"] = flow_module
        setattr(stimma_pkg, "flow", flow_module)
    namespace = _dsl_namespace()
    for name, value in namespace.items():
        setattr(flow_module, name, value)

    return stimma_pkg, flow_module


def _flow_sandbox_workspace() -> Path:
    """A writable scratch dir for the flow build-time sandbox.

    ``build_safe_builtins`` requires a workspace path for its ``open()``
    guard, even though flow programs aren't expected to open files during
...
def build_graph_from_callable(
    fn: Callable[..., Any] | FlowDecorated,
    inputs: Optional[dict[str, Any]] = None,
) -> EquationGraph:
    """Build the graph for a decorated flow function.

    Used by tests and by the file-based loader after it finds the @flow
    function inside program.py.
    """
    if isinstance(fn, FlowDecorated):
        flow_fn = fn.fn
        ...
        declared_inputs = {}
        declared_outputs = {}
    flow_name = getattr(flow_fn, "__name__", "flow")

    inputs = dict(inputs or {})

    # Inputs the flow declares but the caller didn't supply still get lifted
    ...
    with activate_context(ctx):
        with push_frame(ctx, parent_key=flow_name, function_name=flow_name):
            ...
            for name, value in inputs.items():
                ...
                call_kwargs[name] = lift_flow_input(
                    ctx,
                    name,
                    value,
                    type_str=type_str,
            ...
            try:
                result = flow_fn(**call_kwargs)
            except DSLError as exc:
                raise ProgramLoadError(
                    f"flow {flow_name!r} failed during graph construction: {exc}",
                    category="dsl",
    ...
    _eagerly_expand_static_deferreds(graph, flow_name)
    ...
    _validate_all_equations_have_phase(graph, flow_name)
    _validate_no_dangling_equations(graph, flow_name)
    return graph
...
def _collect_return_output_bindings(
    ...
    *,
    ...
    When a flow declares outputs, the return contract is explicit:
    ...
    Flows without declared outputs keep the older permissive behavior so
    ...
    def _single_binding(name: str, value: Any) -> dict[str, str]:
        if not isinstance(value, NodeRef):
            raise ProgramLoadError(
                f"{flow_name}: output {name!r} must be a NodeRef, got {type(value).__name__}",
                category="dsl",
                suggestion=(
    ...
    if len(declared_names) == 1:
        ...
        if isinstance(result, dict):
            ...
            if actual_names != expected_names:
                ...
                raise ProgramLoadError(
                    f"{flow_name}: return dict must match declared outputs exactly ({detail_text})",
                    category="dsl",
                    suggestion=(
                        f"For this flow, either `return {name}` directly or "
                        f"`return {{\"{name}\": {name}}}` with the declared output name."
                    ),
                )
            return _single_binding(name, result.get(name))
        raise ProgramLoadError(
            f"{flow_name}: single-output flows must return a NodeRef or a one-key dict",
            category="dsl",
            suggestion=(
    ...
    if not isinstance(result, dict):
        raise ProgramLoadError(
            f"{flow_name}: flows with multiple declared outputs must return a dict",
            category="dsl",
            suggestion=(
    ...
    for name in declared_names:
        ...
        if existing is not None:
            raise ProgramLoadError(
                f"{flow_name}: outputs {existing!r} and {name!r} both return the same node",
                category="dsl",
                suggestion=(
...
def _collect_legacy_return_output_bindings(result: Any) -> dict[str, str]:
    """Legacy fallback for flows/tests that declare no outputs."""
    seen: set[str] = set()
    bindings: dict[str, str] = {}
...
def _serialize_output_specs(specs: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Turn the @flow(outputs={...}) declaration into a UI-facing schema dict.

    Mirrored into Flow.output_schema by flow_lifecycle so program.py is
...
def _eagerly_expand_static_deferreds(
    graph: EquationGraph, flow_name: str,
) -> None:
    """Materialize any deferred whose iteration count is statically knowable.

    Runs at the end of graph build, after the flow function has returned
    ...
    """
    # Imported lazily — flow_runtime.foreach_expansion imports
    # flow_dsl.context at module load, which would create a cycle if the
    # entry point starts from flow_runtime.engine (engine → expansion →
    # flow_dsl/__init__ → loader → expansion-still-loading).
    from flow_runtime.foreach_expansion import (
        expand_foreach,
    ...
    while True:
        ...
        for deferred in graph.all_deferred():
            ...
            try:
                ...
                raise ProgramLoadError(
                    f"{flow_name}: foreach/hitl.approve {deferred.owner_key!r} "
...
# FLOW_INPUT are structural/declarative and are exempt.
...
# flow's ``return``. ``info`` is narrative commentary and is intentionally
# terminal; ``flow_input`` may be declared while the agent is iterating and
...
def _validate_all_equations_have_phase(graph: EquationGraph, flow_name: str) -> None:
    ...
    raise ProgramLoadError(
        f"{flow_name}: {len(orphans)} equation(s) are outside any phase: {sample}{more}",
        category="dsl",
        suggestion=(
...
def _validate_no_dangling_equations(graph: EquationGraph, flow_name: str) -> None:
    ...
    equation nor surfaced through the flow's ``return``. Typical causes:
    the flow forgot to ``return`` the final node; a call was assigned to a
    ...
    terminal is the whole point. ``flow_input`` is exempt so declaring an
    ...
    raise ProgramLoadError(
        f"{flow_name}: {len(dangling)} equation(s) produce a value nothing uses: {sample}{more}",
        category="dsl",
        suggestion=(
            "Every tool()/llm()/code()/hitl.*()/foreach()/create_* call must "
            "either feed another DSL call or be reachable from the flow's "
...
def build_graph_from_source(
    ...
    *,
    ...
    """Parse and execute a flow source string, build its graph.
    ...
    """
    # Imported lazily to avoid a circular import: agent.v2 → flow_lifecycle
    # → flow_runtime → flow_dsl.loader.
    from agent.v2.code_runtime import ALLOWED_MODULES, build_safe_builtins

    stimma_pkg, flow_module = _ensure_stimma_flow_module_registered()
    ...
    # allow-list. ``stimma`` and ``stimma.flow`` are exposed via
    # ``extra_modules`` so ``from stimma.flow import …`` resolves.
    safe_builtins = build_safe_builtins(
        _flow_sandbox_workspace(),
        _noop_print,
        extra_modules={
            "stimma": stimma_pkg,
            "stimma.flow": flow_module,
        },
    )
    module_globals: dict[str, Any] = {
        "__name__": "_stimma_flow_program",
        "__file__": filename,
    ...
    flow_fn = _find_flow_function(module_globals)
    if flow_fn is None:
        raise ProgramLoadError(
            f"{filename}: no @flow-decorated function found",
            category="dsl",
            suggestion=(
                "Define the flow entry point with @flow(...) — the agent "
                "needs exactly one decorated function per program."
            ),
        )
    return build_graph_from_callable(flow_fn, inputs=inputs)
    ...
    if isinstance(exc, NameError):
        return ProgramLoadError(
            ...
            suggestion=(
                ...
                "before @flow. If it's a DSL primitive, check the spelling."
            ),
        )
...
def _find_flow_function(namespace: dict[str, Any]) -> Optional[FlowDecorated]:
    flows = [
        value for value in namespace.values()
        if isinstance(value, FlowDecorated)
    ]
    if len(flows) > 1:
        names = ", ".join(r.__name__ for r in flows)
        raise ProgramLoadError(
            f"program.py defines multiple @flow functions: {names}",
            category="dsl",
            suggestion=(
                "A flow program must define exactly one @flow(...) entry "
                "point. Split smoke tests into separate flows/files, or keep "
                "only the flow you want to run."
            ),
        )
