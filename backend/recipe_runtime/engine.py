"""Recipe execution engine — the FRP scheduler.

Owns one `RecipeRun` per running recipe. Responsibilities:
...
the recipe's cap (RECIPES_TECH §FRP Runtime).

All state changes write through to the per-recipe state.db via
`graph_db.py`.
"""
...
@dataclass
class RecipeRunConfig:
    """Fixed configuration for a single RecipeRun (never mutated)."""
    recipe_id: int
    state_db_path: Path
    ...
    # runtime/engine uses this to emit recipe_task_created /
    # recipe_task_resolved / recipe_equation_updated events. Defaults to a
...
class RecipeRun:
    """Execution state for a single running recipe.

    Usage:
        run = RecipeRun(graph, config, evaluators=registry, store=store)
    ...
    async def start(self) -> None:
        ...
        if self._state == RunState.RUNNING:
            log.warning(
                "recipe %s: scheduler was marked RUNNING but task is dead; restarting",
    ...
    async def pause(self) -> None:
        ...
        written through to state.db on completion). Per RECIPES_TECH
        §Pause / Resume.
        """
        if self._state != RunState.RUNNING:
            return
    ...
    def _start_loop_task(self) -> None:
        self._task = asyncio.create_task(
            self._run_loop(), name=f"recipe-run-{self.config.recipe_id}"
        )
    ...
    def _on_loop_done(self, task: asyncio.Task[None]) -> None:
        ...
        if exc is None:
            return
        log.exception(
            "recipe %s: scheduler task exited unexpectedly",
            self.config.recipe_id,
            exc_info=exc,
        )
        if self._state == RunState.RUNNING:
    ...
    def _subscribe_tools_changed(self) -> None:
        ...
        try:
            ...
            log.exception("failed to subscribe to tools-changed for recipe %s",
                          self.config.recipe_id)
    ...
    def _handle_tools_changed(self) -> None:
        ...
        for eq in self.graph.all_equations():
            ...
            reset_any = True
            log.info(
                "recipe %s: tool %s now available — resetting %s to PENDING",
                self.config.recipe_id, tool_id, eq.key,
            )
        if reset_any:
            self._wakeup.set()
    ...
    def _reset_stale_computing(self) -> None:
        ...
        never scheduled by this RecipeRun instance (i.e. left behind by a
        ...
        for key in to_reset:
            ...
            log.info(
                "recipe %s: reset stale COMPUTING equation %s to PENDING",
                self.config.recipe_id, key,
            )
    ...
    def recover_orphaned_work(self) -> None:
        ...
        in-memory equations in COMPUTING even though this RecipeRun has no
    ...
    def _is_quiescent(self) -> bool:
        ...
        # early and external observers (tests, HTTP) see the recipe as
    ...
    async def resolve_task(self, task_id: int, resolution: Any) -> None:
        ...
        # FAILED equation with retry/skip/edit-recipe affordances, instead
        ...
        if record is None:
            ...
            # state.db while this new RecipeRun has no in-memory PendingHitl
        ...
        if record.task_type == "approve" and resolution is False:
            ...
            # Durable per-recipe HITL result.
            graph_db.insert_hitl_result(
                self.config.state_db_path,
                eq.key,
                record.inputs_hash,
    ...
    async def resolve_error_task(
        ...
        *,
        ...
        Actions:
            ...
            edit_recipe   — no-op on the runtime; just mark task resolved
        """
        task_row = graph_db.get_task(self.config.state_db_path, task_id)
        if task_row is None:
        ...
        if task_type == "waiting_for_tool" and action not in {"skip", "edit_recipe"}:
            raise ValueError(
                f"unknown waiting-for-tool action {action!r}; expected skip | edit_recipe"
            )
    ...
    def _reset_equation_to_pending(
        ...
        *,
        ...
        if (
            eq.equation_type == EquationType.RECIPE_INPUT
            and not eq.is_waiting_for_recipe_input()
        ):
    ...
    async def _run_loop(self) -> None:
        try:
            ...
            log.exception("recipe run loop crashed")
            raise
    ...
    async def _schedule_ready(self) -> int:
        ...
        for eq in ready[:available]:
            ...
            if eq.equation_type == EquationType.CONTROL or eq.equation_type == EquationType.RECIPE_INPUT:
                self._complete_control_equation(eq)
                scheduled += 1
                continue
    ...
    async def _evaluate_with_cap(self, eq: Equation) -> None:
        ...
        async with self._sem:
            try:
                ...
                log.exception(
                    "recipe %s: evaluator task crashed for %s",
                    self.config.recipe_id,
                    eq.key,
                )
                if eq.status == EquationStatus.COMPUTING:
    ...
    async def _evaluate_equation(self, eq: Equation) -> None:
        ...
        # HITL path: check per-recipe hitl_results first.
        if eq.equation_type == EquationType.HITL:
        ...
        if hit is not None:
            ...
            # restore — the recipe is using them again.
        ...
        if last_exc is not None:
            if category == RESOURCE_ERROR:
                ...
                log.warning(
                    "recipe %s: resource exhaustion, pausing", self.config.recipe_id
                )
                await self.pause()
                return
            self._finalize_failure(eq, str(last_exc), category=category)
        ...
        # Persist to store and per-recipe DB.
    ...
    def _create_error_task(
        ...
        *,
        ...
        Error tasks carry enough context for the user to decide:
          ...
          - available actions (retry, skip if in a foreach, edit_recipe)
        """
        # Replace any prior pending task on this equation. Without this, each
        # retry accumulates a fresh row and the recipe shows the same error N
        # times — tasks represent current recipe state, not history, so a new
    ...
    def _cancel_and_broadcast_pending_tasks(
        ...
        accumulate across retries and the per-recipe pending-task count
    ...
    def _broadcast_task_resolved(
        ...
        self._fire_broadcast("recipe_task_resolved", payload)
    ...
    def _broadcast_equation_updated(self, equation_key: str) -> None:
        """Fire recipe_equation_updated for the current in-memory equation.
    ...
    def _persist_status(
        ...
        **kwargs: Any,
        ...
        """Write a status change to state.db and broadcast recipe_equation_updated.
    ...
    def _complete_control_equation(self, eq: Equation) -> None:
        """Control equations (recipe_input, literal_list, foreach wrapper,
    ...
    def _collect_foreach_result(self, wrapper: Equation) -> list[Any]:
        ...
        Skipped iterations (per RECIPES_TECH §Failure Isolation and §User
    ...
    def _expand_foreach(
        ...
        *,
        ...
        Thin wrapper over :mod:`recipe_runtime.foreach_expansion`. The pure
...
_ERROR_ACTIONS_IN_LOOP = ["retry", "skip", "edit_recipe"]
_ERROR_ACTIONS_NOT_IN_LOOP = ["retry", "edit_recipe"]
...
def _is_inside_foreach_iteration(equation_key: str) -> bool:
    ...
    sits inside (or is) a foreach-iteration wrapper. The RECIPES_EQUATION_KEYS
...
def _iteration_prefix_for(equation_key: str) -> Optional[str]:
    ...
    # Walk from the outside in; the first segment is the recipe-function
