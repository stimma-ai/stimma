...
See docs/RECIPES_EQUATION_KEYS.md §4 and docs/RECIPES_DSL.md §7. In short:
...
Where:
  - `equation_type` is one of 'tool_call', 'llm_call', 'code'. HITL
    ...
    per-recipe hitl_results table keyed on (equation_key, inputs_hash).
...
Static vs dynamic parameter boundary:
  - static = value known at graph-build time (literals in the program,
    recipe inputs that are scalars/enums) -> contributes to definition_hash
...
def definition_hash_for_llm_slot(batch_key: str, slot_index: int) -> str:
    ...
    batches (different prompts / different recipes) never share slot keys.
    """
...
def inputs_hash_for_values(resolved_inputs: Mapping[str, Any]) -> str:
    ...
    IDs are installation-specific and would break cross-recipe sharing.
    """
...
def compute_store_key(
    ...
    We keep the exact structure specified in RECIPES_EQUATION_KEYS.md §4
    ...
    if equation_type not in STOREABLE_EQUATION_TYPES:
        raise StoreKeyError(
            ...
            f"equation store (HITL and control equations are per-recipe)"
        )
    if attempt < 1:
...
def derive_seed(equation_key: str, attempt: int) -> int:
    ...
    `seed = hash(equation_key + attempt)` per RECIPES_TECH.md §Store Keys.
