...
``recipe_runtime.evaluators.Evaluator`` for one equation type and raises
``EvaluatorError`` with the correct category so the engine's retry / pause /
error-task flow does the right thing (see RECIPES_TECH.md §Error Handling):
  ...
  - ``tool_call``  → generation queue + ``execute_call_tool`` path used by
    ...
    with ``{source: "recipe", recipe_id, equation_key, phase_path}`` so Phase 6
...
def _is_framework_frame(filename: str) -> bool:
    ...
    package — i.e. not something the user wrote in their recipe."""
    if not filename:
        return True
...
def _format_user_code_error(exc: BaseException) -> str:
    ...
    The agent needs the failing line number to debug a recipe — the bare
...
async def _extract_compute_duration_ms(media_ids: list[int]) -> Optional[int]:
    ...
    recipe runtime surfaces this so per-iteration UI can show compute time
...
async def _tag_media_with_recipe(
    media_ids: list[int],
    *,
    ...
    """Merge a recipe tag into each produced MediaItem.generation_metadata.
    ...
    """
    if not media_ids or recipe_id is None:
        return
...
async def restore_trashed_media_silently(media_ids: list[int]) -> None:
    ...
    Called from the recipe engine's store-hit path: when an equation
...
def _validate_tool_inputs(tool_id: str, inputs: dict[str, Any]) -> None:
    ...
    the tool's own parameter_schema at the recipe boundary turns these into a
...
def _normalize_tool_media_inputs(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    with ``media_id`` attached after URL-media promotion. Recipe authors should
...
class ToolCallEvaluator:
    ...
    def __init__(
        ...
        *,
        ...
        self._tag_media = tag_media or _tag_media_with_recipe
    ...
    async def __call__(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        try:
            ...
            log.exception(
                "recipe media tagging failed for %s (recipe=%s)",
...
# forever when a provider wedges. Recipe prompts are almost always short
...
class LLMEvaluator:
    ...
    async def __call__(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        # Recipe LLM calls default to no-thinking — most are short classification
        ...
        try:
            ...
            log.info(
                "recipe llm[%s] role=%s think=%s start: %r",
            ...
            log.info(
                "recipe llm[%s] role=%s think=%s done",
                request.equation_key, role, think,
            )
        except asyncio.TimeoutError as exc:
            log.warning(
                "recipe llm[%s] role=%s timed out after %.0fs",
    ...
    async def _build_image_blocks(self, binding: Any) -> list[dict[str, Any]]:
        ...
        recipe's fallback / retry behavior kicks in instead of silently
...
# batch result (see RecipeRun._invalidate_equation).
...
class LLMBatchEvaluator(LLMEvaluator):
    ...
    async def _run_llm(
        ...
        try:
            ...
            completion = await self._get_completion()
            log.info(
                "recipe llm_batch[%s] role=%s think=%s start",
            ...
            log.info(
                "recipe llm_batch[%s] role=%s think=%s done",
                equation_key, role, think,
            )
        except asyncio.TimeoutError as exc:
            raise EvaluatorError(
...
class LLMSlotEvaluator(LLMEvaluator):
    ...
    def set_graph_getter(self, graph_getter: Callable[[], Any]) -> None:
        ...
        ``RecipeRuntime`` wires this on construction.
        """
        self._graph_getter = graph_getter
    ...
    async def __call__(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        try:
            ...
            completion = await self._get_completion()
            log.info(
                "recipe llm_slot[%s] solo-gen role=%s attempt=%s",
...
def _default_workspace_root() -> Path:
    ...
    ``_make_safe_open`` for workspace-scoped file access). Recipes don't have
    a persistent workspace, so we use a subdirectory under the recipes root
...
async def _run_code_snippet(
    ...
    """Execute a recipe ``code()`` snippet in the agent's sandbox primitives.
    ...
    tracker, LLM usage capture — recipe code doesn't need those. Inputs bind
    ...
    # Swallow prints — recipes aren't a chat context.
    ...
    wrapper = "async def __recipe_code__():\n"
    for line in body_source.splitlines() or [""]:
        wrapper += f"    {line}\n"

    globals_dict: dict[str, Any] = {
        "__builtins__": builtins,
        "__name__": "__recipe_code__",
    }
    globals_dict.update(inputs)

    def _compile() -> Callable[..., Any]:
        exec(compile(wrapper, "<recipe_code>", "exec"), globals_dict, globals_dict)
        return globals_dict["__recipe_code__"]
...
async def _invoke_code_callable(fn: Callable[..., Any], inputs: dict[str, Any]) -> Any:
    ...
    graph construction (see ``recipe_dsl.loader.build_graph_from_source``),
...
class CodeEvaluator:
    ...
    (see ``recipe_runtime.engine._resolve_dynamic_bindings``). Those names
    bind as globals in the sandboxed frame. Any exception from the snippet
    surfaces as CODE_ERROR — the user's repair path is `retry` / `edit recipe`.
    """
...
def _coerce_media_id(value: Any) -> int:
    ...
    Recipe graphs sometimes carry single media ids as dicts (e.g., from a
...
async def _explode_previous_recipe_assembly(
    *,
    recipe_id: Optional[int],
    equation_key: str,
) -> None:
    """Release members + delete any prior set/grid this recipe-equation created.

    The recipe "owns" its create_set/create_grid output — on re-run (user
    ...
    Find prior assemblies by the recipe tag we stamp in
    ``_tag_media_with_recipe``, unsupersede their members, drop the file,
    and soft-delete the row. No-op when recipe_id is None (unit tests with
    ...
    async with _open_session() as session:
        result = await session.execute(
            select(MediaItem).where(
                ...
                func.json_extract(MediaItem.generation_metadata, "$.source") == "recipe",
                func.json_extract(MediaItem.generation_metadata, "$.recipe_id") == recipe_id,
    ...
    log.info(
        "recipe %s eq %s: exploded %d prior assembl%s, freed %d members",
        recipe_id,
        equation_key,
...
class CreateSetEvaluator:
    ...
    Before assembling we explode any set this recipe-equation produced on a
    prior run. The recipe owns its one output set; a re-run replaces it
    ...
    async def __call__(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        await _explode_previous_recipe_assembly(
            recipe_id=request.recipe_id,
            equation_key=request.equation_key,
        )
...
class CreateGridEvaluator:
    ...
    ``CreateSetEvaluator`` — the recipe owns its one grid; a re-run
...
async def _save_document_media(
    *,
    ...
    because recipes are the only caller today — no point adding a new
    surface to the STP agent tool registry for a recipe-only capability.
    """
...
def _shape_is_media(shape: Any) -> bool:
    """Duck-type check for Scalar(kind='media') without importing recipe_dsl."""
...
async def _load_media_row(media_id: int) -> tuple[str, str]:
    ...
    file path is empty. Single source of truth for the recipe evaluators'
    media lookups.
    """
    from database import MediaItem
...
class CreateLayoutEvaluator:
    ...
    async def __call__(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        try:
            ...
            #     fail loudly with a CODE_ERROR so the recipe author sees it.
            ...
            if missing:
                ...
                raise EvaluatorError(
                    ...
                    f"bogus list-literal. If you're fanning a recipe input "
...
class WebSearchEvaluator:
    ...
    valid output (the recipe author can decide whether to error out
...
async def promote_url_media_value(
    ...
    *,
    ...
    the boundary where a descriptor becomes part of recipe state (select,
