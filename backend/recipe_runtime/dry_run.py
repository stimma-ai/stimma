"""Dry-run/preflight execution for recipe programs.
...
The goal is to catch structural recipe bugs before spending real tool/LLM
...
from recipe_dsl.loader import build_graph_from_program_file
...
from .state_db import create_recipe_state_db
...
async def dry_run_recipe(
    *,
    ...
    """Build and execute a recipe against synthetic providers."""
    cfg = config or DryRunConfig()
    start = perf_counter()
    sampled_inputs, sampled_counts = sample_recipe_inputs(
        inputs, max_items=cfg.max_items_per_collection,
    )

    with tempfile.TemporaryDirectory(prefix=f"stimma-recipe-dry-run-{recipe_id}-") as td:
        root = Path(td)
...
async def _wait_with_budgets(run: RecipeRun, cfg: DryRunConfig) -> None:
        ...
        value=[
            {
                ...
                "snippet": "Synthetic search result for recipe dry-run.",
            }
        ]
    )
