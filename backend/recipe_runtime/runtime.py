...
Wires the in-memory `EquationGraph` + `RecipeRun` to the recipe's
per-recipe state.db and program.py. Intended to be the single entry point
other parts of the backend use to start/pause/resume/invalidate/edit a
recipe.

One `RecipeRuntime` per recipe. Tests construct them directly; production
...
from recipe_dsl.loader import (
    build_graph_from_callable,
    build_graph_from_program_file,
    load_program_with_error_classification,
)
from recipe_dsl.versions import ProgramVersionStore, get_version_store

from . import graph_db
from .engine import RecipeRun, RecipeRunConfig
...
class RecipeRuntime:
    """Owns the graph + scheduler for one recipe.

    Typical lifecycle:

        runtime = RecipeRuntime(recipe_id, state_db_path, program_path, inputs,
                                evaluators=registry)
    ...
    def __init__(
        ...
        *,
        ...
        recipe_callable: Optional[Callable[..., Any]] = None,
        ...
        if program_path is None and recipe_callable is None:
            raise ValueError("program_path or recipe_callable is required")
        self.recipe_id = recipe_id
        self.project_id = project_id
        self.state_db_path = state_db_path
        self.program_path = program_path
        self.recipe_callable = recipe_callable
    ...
    def _settle_recipe_input_equations(self, graph: EquationGraph) -> None:
        """Eagerly complete recipe inputs whose values are already present.

        The scheduler also completes RECIPE_INPUT control equations, but a
        recipe can have a live graph while idle or paused (e.g. after a
        ...
        for eq in graph.all_equations():
            if eq.equation_type != EquationType.RECIPE_INPUT:
                continue
            if eq.is_waiting_for_recipe_input():
                continue
    ...
    def build_initial_graph(self, inputs: Optional[dict[str, Any]] = None) -> EquationGraph:
        ...
        if self.recipe_callable is not None:
            graph = build_graph_from_callable(self.recipe_callable, inputs=self.inputs)
        else:
        ...
        if orphan_keys:
            ...
            log.info(
                ...
                "for recipe %s (program edited between runtime instances)",
                len(orphan_keys), self.recipe_id,
            )
        self.graph = graph
        ...
        self._settle_recipe_input_equations(graph)
    ...
    def try_reload_program(self) -> tuple[Optional[GraphDiff], Optional[ProgramLoadError]]:
        ...
        try:
            if self.recipe_callable is not None:
                new_graph = build_graph_from_callable(
                    self.recipe_callable, inputs=self.inputs
                )
            else:
        ...
        # recipe_task_resolved event fires, so the denormalized
        ...
        # per-recipe task views don't drift.
        ...
        # Fire a recipe_task_resolved for each stale task so the pending
        # counter and per-recipe task view refresh. We do this after the DB
        # writes settle so any listener that re-reads state.db sees the
        # cancelled rows.
        for task_row in tasks_to_notify:
            self._broadcast_now(
                "recipe_task_resolved",
                {
                    "recipe_id": self.recipe_id,
        ...
        self._settle_recipe_input_equations(new_graph)
    ...
    def _broadcast_phase_updates(
        ...
        """Broadcast recipe_phase_updated for each distinct phase touched by diff.
    ...
    async def resolve_task(self, task_id: int, resolution: Any) -> None:
        if self.run is None:
            raise RuntimeError("resolve_task: RecipeRun not started")
    ...
    async def resolve_error_task(
        ...
        if self.run is None:
            raise RuntimeError("resolve_error_task: RecipeRun not started")
...
def recover_all_running_recipes(
    recipe_rows: list[dict[str, Any]],
    build_runtime: Callable[[dict[str, Any]], RecipeRuntime],
) -> list[RecipeRuntime]:
    """Reset `computing` rows to `pending` and return runtimes ready to start.

    Called at backend startup (RECIPES_TECH §Implementation Notes). Caller
    supplies `recipe_rows` (list of row-dicts with at least id and
    state_db_path accessible), and a builder that constructs a configured
    `RecipeRuntime` for each.
    ...
    previously-running recipes paused until the user confirms).
    """
    runtimes: list[RecipeRuntime] = []
