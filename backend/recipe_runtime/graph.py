...
Persistence to the per-recipe state.db is handled by graph_db.py. This
...
class EquationType(str, Enum):
    ...
    CODE = "code"
    HITL = "hitl"
    CONTROL = "control"  # wrapper / foreach / zip / recipe_input
...
# a bug. See RECIPES_TECH.md §FRP Runtime for the lifecycle.
_STATUS_TRANSITIONS = {
...
class NodeRef:
    ...
    f-string interpolation all raise NodeUsageError. This matches RECIPES_DSL
    ...
    # iteration keys (e.g. a scalar recipe-input list), those keys are
    ...
    # Shape the node will resolve to (recipe_dsl.shapes.Shape). Populated by
    ...
    def _reject(self, op: str, *, hint: str = "") -> "None":
        # Deferred import: recipe_dsl depends on recipe_runtime, not the
        # other way around.
...
class Equation:
    ...
    def is_waiting_for_recipe_input(self) -> bool:
        """True when this recipe input node has not been provided yet.
        ...
        """
        if self.equation_type != EquationType.RECIPE_INPUT:
            return False
        if self.definition.get("control_kind") != "recipe_input":
            return False
...
class EquationGraph:
    ...
    def __init__(self) -> None:
        ...
        # Root equation key (what the recipe function returned), set by the
        # graph builder. None for recipes that don't return anything. For
        ...
        # All equations the recipe's ``return`` statement surfaces as outputs.
        ...
        # per NodeRef. Empty when the recipe returns nothing (or a non-node
        # value). The recipe UI uses this to decide which equations get a
        ...
        # Reverse lookup for named outputs declared via @recipe(outputs=...).
        # Maps surfaced equation key -> declared output name. Single-output
        # recipes that return a bare NodeRef still populate this mapping with
        # that lone declared name.
        self.output_name_by_key: dict[str, str] = {}
        # UI-serialized mirror of the @recipe(inputs={...}) declaration, set
        # by the graph builder. Shape: {name: {type, description?, default?,
        # options?, lines?}}. recipe_lifecycle writes this into
        # Recipe.input_schema so the input form stays in sync with program.py.
        self.input_specs: dict[str, Any] = {}
        # Same idea for @recipe(outputs={...}); shape: {name: {type,
        # description?}}. Mirrored into Recipe.output_schema.
    ...
    def ready_equations(self) -> list[Equation]:
        ...
        parent foreach's collection; see RECIPES_TECH §Failure Isolation).
