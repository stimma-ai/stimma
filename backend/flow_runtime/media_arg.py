"""Unified ``Media`` wrapper for flow-sandbox callable inputs.
...
to import ``stimma.library`` from the flow sandbox to resolve an id).
...
returns ``id`` so legacy flows that used a media-id int as a dict key
