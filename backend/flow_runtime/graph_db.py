"""Per-flow state.db persistence for the equation graph.

The in-memory `EquationGraph` is the working copy during an active run. All
state transitions also write through to the per-flow state.db so that
...
def sync_equation_definitions(db_path: Path, equations: Iterable[Equation]) -> None:
    ...
    (e.g. a flow input gained ``optional=True``, a ``foreach(...)`` moved
...
def list_tasks(
    ...
    *,
    ...
    cross-flow listing fans out across many flows and one broken file
...
def clear_flow_state(db_path: Path) -> None:
    """Delete persisted equations, tasks, and HITL results for a flow."""
    conn = _connect(db_path)
    try:
