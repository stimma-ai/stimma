...
# because their result is unknown — the flow was interrupted mid-evaluation.
...
def create_flow_state_db(db_path: Path) -> None:
    """Create the per-flow state.db at `db_path` with schema + WAL mode.
