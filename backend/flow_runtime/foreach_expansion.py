...
panel; the loader expands into an in-memory graph and lets the flow
...
class ExpansionPersistor:
    ...
    ``FlowRun._persist_status``, and ``FlowRun._finalize_failure``.
...
def iteration_keys_from_upstream(
    ...
    if kind in ("flow_input", "literal_list"):
...
def find_eager_expansion(
    ...
    upstream is a flow input that hasn't settled, etc.).

    Eligible cases:
      - ``hitl.approve`` — input is the synthetic ``literal_list`` of slot
        indices; values are statically known.
      - ``foreach`` over a literal collection (flow-input list, literal
            ...
            for iter_key, item in zip(iter_keys, items):
                ...
                with push_frame(
                    ...
                    if is_approve and isinstance(ret_node_or_value, NodeRef):
                        from flow_dsl.primitives import _register_per_slot_approve
