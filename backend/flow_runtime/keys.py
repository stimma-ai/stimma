...
# (reserved for sub-flow namespace) and any '$' (reserved for positional
...
def validate_function_name(name: str) -> None:
    ...
    if name.startswith(_SUBFLOW_PREFIX):
        raise EquationKeyError(
            f"function name {name!r} starts with reserved '@' (sub-flow prefix)"
        )
...
_EQUATION_KEY_ALLOWED_RE = re.compile(
    r"^[A-Za-z_][A-Za-z0-9_.@]*"                 # root function name (or @sub-flow)
