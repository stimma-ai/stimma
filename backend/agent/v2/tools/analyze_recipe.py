"""analyze_recipe — read the current recipe's runtime state.

Mirrors the data the frontend gets from /recipes/{id}/phases, /equations, and
...
@tool(
    name="analyze_recipe",
    description=(
        "Read the current state of this recipe — phase progress, errors, "
...
async def analyze_recipe(
    ...
    **kwargs,
    ...
    if session is None or chat_id is None:
        return "Error: analyze_recipe requires an active chat session."
    ...
    if chat.recipe_id is None:
        return (
            "Error: this chat is not attached to a recipe. analyze_recipe is "
            "only usable in recipe chats."
        )
    ...
    # Late imports: routes.recipes pulls in FastAPI + many backend modules,
    # and the agent's recipe_runtime deps want to initialise lazily.
    from routes.recipes import (
        _apply_runtime_overlay,
        _definition_from_runtime,
        _display_name_for_equation,
        _load_equations_for_recipe,
    )

    rows = _apply_runtime_overlay(
        _load_equations_for_recipe(recipe.id), recipe.id,
    )
    for row in rows:
        row["definition"] = _definition_from_runtime(recipe.id, row["equation_key"])
        row["display_name"] = _display_name_for_equation(
            row["equation_type"], row["definition"],
        )

    if scope == "overview":
        return _format_overview(recipe, rows)
    if scope == "phase":
        if not name:
            return (
                "Error: scope='phase' requires `name` (the phase path). "
                "Call analyze_recipe() with no arguments to see available phases."
            )
    ...
    if scope == "equation":
        if not name:
            return (
                ...
                "Call analyze_recipe() with no arguments to see failed keys."
            )
        return _format_equation(rows, name, recipe_id=recipe.id)
    if scope == "tasks":
        return _format_tasks(recipe)
...
def _format_overview(recipe: Recipe, rows: list[dict[str, Any]]) -> str:
    lines: list[str] = []
    lines.append(f'Recipe: "{recipe.name}"  — state: {recipe.execution_state}')
    lines.append(f"Inputs: {_fmt_inputs(recipe)}")
    lines.append("")
    ...
    load_err = _current_load_error(recipe.id)
    if load_err is not None:
        ...
        ctx = _program_context_for_traceback(recipe.id, msg)
        if ctx:
            lines.append("")
    ...
    # "(root)" bucket when it only holds recipe_input scaffolding — those are
    ...
    def is_input_only(phase_rows: list[dict[str, Any]]) -> bool:
        return all(r["equation_type"] == "recipe_input" for r in phase_rows)
    ...
    if fails and load_err is None:
        ...
        lines.append(
            "Next step: drill into each with analyze_recipe(scope='equation', "
        ...
        for r in fails[:10]:
            ...
            lines.append(
                f"    → analyze_recipe(scope='equation', name={r['equation_key']!r})"
            )
        if len(fails) > 10:
    ...
    if awaiting:
        ...
        lines.append("  → analyze_recipe(scope='tasks')")
        lines.append("")

    lines.append("Drill in:")
    lines.append("  analyze_recipe(scope='phase',    name='<phase-name>')")
    lines.append("  analyze_recipe(scope='equation', name='<equation-key>')")
    lines.append("  analyze_recipe(scope='tasks')")
    return "\n".join(lines)
...
def _format_phase(rows: list[dict[str, Any]], phase_arg: str) -> str:
    ...
    if failed_keys:
        ...
        for k in failed_keys[:3]:
            lines.append(f"  analyze_recipe(scope='equation', name={k!r})")
    return "\n".join(lines)
...
def _format_equation(
    rows: list[dict[str, Any]], key: str, recipe_id: int | None = None,
) -> str:
    row = next((r for r in rows if r["equation_key"] == key), None)
    if row is None:
        return (
            f"Error: no equation with key {key!r}. Call analyze_recipe() with "
    ...
    if row.get("error"):
        ...
        if recipe_id is not None:
            ctx = _program_context_for_traceback(recipe_id, err_text)
            if ctx:
                lines.append("")
                lines.append("Program.py context around the failure:")
                lines.extend(ctx)
...
def _append_definition_block(
    ...
    # Fallback: compact key/value summary for hitl / info / recipe_input /
...
def _program_context_for_traceback(
    ...
    """
    try:
        from recipe_runtime.paths import get_recipe_program_path
    except ImportError:
        return []
...
def _extract_noderef_key(v: Any) -> str | None:
    ...
    try:
        from recipe_runtime.graph import NodeRef  # lazy — runtime module
    except ImportError:
...
def _format_tasks(recipe: Recipe) -> str:
    import recipe_registry
    from recipe_runtime import graph_db
    from recipe_runtime.paths import get_recipe_state_db_path

    runtime = recipe_registry.get_runtime(recipe.id)
    if runtime is not None:
        rows = runtime.list_tasks(status="pending")
    else:
        state_db = get_recipe_state_db_path(recipe.id)
...
def _current_load_error(recipe_id: int) -> dict[str, Any] | None:
    """Return the most recent program-load error for this recipe, or None.
    ...
    cached = recipe_lifecycle.get_cached_load_error(recipe_id)
    return cached
...
def _fmt_inputs(recipe: Recipe) -> str:
    schema = _loads_or_empty(recipe.input_schema)
    values = _loads_or_empty(recipe.inputs)
    if not schema:
...
def _summarize_definition(etype: str, d: dict[str, Any]) -> dict[str, str]:
    ...
    for everything else (hitl, info, recipe_input, control, create_*).
    """
    out: dict[str, str] = {}
    if etype == "tool_call":
