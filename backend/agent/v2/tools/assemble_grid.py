...
@tool(
    ...
    description=(
        ...
        "Cell images are superseded. This is a deliberate comparison artifact the user explicitly "
        "asks for ('grid', 'sweep', 'compare X across Y'); the parameter-grid stimpack owns the "
        "workflow and confirms the sweep axes with the user before generating, so invoke that "
        "stimpack first and let it drive. To present several ordinary results instead, use show()."
    ),
    parameters=[
        ToolParameter(
            name="media_ids",
...
async def create_parameter_sweep(
    ...
    **kwargs,
    ...
    # Record lineage + supersede cells
    ...
    if cell_media_ids:
        ...
        result = await session.execute(
            select(MediaItem).where(MediaItem.id.in_(cell_media_ids))
        )
        cell_items = list(result.scalars().all())
        if cell_items:
    ...
    return (
        ...
        f"Media ID: {grid_media_item.id}. Cell images are now superseded by the grid."
    )
