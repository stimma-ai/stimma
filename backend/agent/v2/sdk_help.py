...
Gotchas:
  ...
  - Always await; safe for concurrent use inside asyncio.gather.""",
    ...
    "create_parameter_sweep": SDKMethodHelp(
        ...
        summary="Assemble a labeled comparison grid (.stimmagrid.json) from a parameter sweep the user requested. Owned by the parameter-grid stimpack — invoke that stimpack first; it confirms the sweep axes before any generation.",
        details="""\
...
parameter-grid stimpack owns the end-to-end workflow — gathering the axes,
confirming the plan with the user, then assembling — so invoke that stimpack
...
_GROUP_DESCRIPTIONS = {
    ...
    "library": "search, browse, get, save, lineage, sets; parameter-sweep grids via the parameter-grid stimpack",
