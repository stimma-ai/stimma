"""HITL + error tasks API.
    ...
    GET  /api/recipes/:id/tasks      — per-recipe listing (routed from recipes.py)

Task data lives in per-recipe state.db files. Resolution takes a composite
`recipe_id:task_id` so the route stays recipe-agnostic without needing a
...
from models.api_models import RecipeTaskResponse, TaskResolveRequest
...
router = APIRouter(prefix="/api/tasks", tags=["recipe-tasks"])


def _parse_composite_task_id(raw: str) -> tuple[int, int]:
    """Decode a composite task id of the form `{recipe_id}:{task_id}`.

    Task IDs live inside per-recipe SQLite files and aren't globally
    ...
    if ":" not in raw:
        raise HTTPException(
            status_code=400,
            detail=(
                f"task id {raw!r} must be 'recipe_id:task_id' "
                f"(e.g. '42:7')"
            ),
        )
...
def _composite_task_id(recipe_id: int, task_id: int) -> str:
    return f"{recipe_id}:{task_id}"


_VALID_ERROR_ACTIONS = {"retry", "skip", "edit_recipe"}
_VALID_WAITING_FOR_TOOL_ACTIONS = {"skip", "edit_recipe"}
_HITL_TASK_TYPES = {"select", "approve"}


def _task_to_response(row: dict[str, Any]) -> RecipeTaskResponse:
    ...
    # recipes. The numeric per-recipe id is kept in payload for debugging.
    composite = _composite_task_id(
        int(row["recipe_id"]) if row.get("recipe_id") is not None else 0,
        int(row["task_id"]),
    )
    return RecipeTaskResponse(
...
async def _find_task_recipe(
    session: AsyncSession, composite_id: str,
) -> tuple[Recipe, int, dict[str, Any]]:
    """Locate the recipe_id + row for a composite task id."""
    recipe_id, tid = _parse_composite_task_id(composite_id)
...
@router.post("/{task_id}/resolve", response_model=RecipeTaskResponse)
async def resolve_task(
    task_id: str,
    body: TaskResolveRequest,
    session: AsyncSession = Depends(get_db_session),
):
    recipe, tid, task_row = await _find_task_recipe(session, task_id)
    recipe_id = recipe.id
    ...
    if runtime is None:
        ...
        except Exception:  # noqa: BLE001 — idle recipes may still resolve HITL offline
            log.exception("task resolution runtime inflation failed", recipe_id=recipe_id)
            runtime = None

    if runtime is not None and runtime.run is None and recipe.execution_state == "running":
        try:
            await runtime.start()
        except Exception as exc:  # noqa: BLE001
            raise HTTPException(
                status_code=500, detail=f"failed to resume recipe runtime: {exc}",
            ) from exc
    ...
    if task_type == "error":
        ...
        if runtime is None:
            raise HTTPException(
                status_code=409,
                detail=(
                    f"recipe {recipe_id} is not running; error-task actions "
        ...
        if runtime is None or runtime.run is None:
            ...
            # directly. The next recipe start will pick up the stored result
            ...
            if task_type in {"select", "approve"} and not isinstance(resolution, bool):
                try:
                    ...
                    from recipe_runtime.production_evaluators import promote_url_media_value
                    resolution = await promote_url_media_value(
                        resolution,
                        recipe_id=recipe_id,
                        project_id=getattr(recipe, "project_id", None),
                    ...
                    raise HTTPException(status_code=400, detail=str(exc)) from exc
            _resolve_offline(recipe_id, tid, task_row, resolution, task_type)
        else:
            try:
...
def _resolve_offline(
    ...
    """Resolution path when no RecipeRuntime is in memory.
    ...
    """
    from recipe_runtime import graph_db as _gdb
    if task_type == "approve" and resolution is False:
        raise HTTPException(
            status_code=409,
            detail=(
                f"recipe {recipe_id} is not running; approve-rejection "
