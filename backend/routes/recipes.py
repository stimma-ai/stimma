"""Recipe CRUD and fork routes.

See docs/RECIPES_TECH.md §API. Execution, DSL, and HITL resolution live
...
from models.api_models import (
    ...
    RecipeCreateRequest,
    RecipeEquationResponse,
    RecipeForkRequest,
    RecipePhaseNodeResponse,
    RecipePhaseTreeResponse,
    RecipeResponse,
    RecipeTaskResponse,
    RecipeUpdateRequest,
)
...
def _json_safe_recipe_value(value: Any) -> Any:
    """Coerce live recipe values into JSON-safe API payloads.
    ...
    code can return Media wrappers that are useful inside the recipe sandbox
    ...
    if isinstance(value, dict):
        return {str(k): _json_safe_recipe_value(v) for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_json_safe_recipe_value(v) for v in value]
...
def _root_status_summary_for_recipe(recipe_id: int) -> dict[str, int]:
    """Root-phase status_summary for a recipe, matching phase-tree filtering.
    ...
    by the equation-update broadcast so the sidebar / recipes page / project
    overview can derive `Running / Done / Your Turn / Error` without having
    to open the recipe to hydrate the summary cache.
    """
    ...
    rows = _load_equations_for_recipe(recipe_id)
    if not rows:
        return {}
    return compute_root_summary_from_rows(rows)


def _recipe_has_load_error(recipe_id: int) -> bool:
    """Return True when the recipe's program is currently parked behind a
    ...
    return recipe_lifecycle.get_cached_load_error(recipe_id) is not None


def _serialize(recipe: Recipe) -> RecipeResponse:
    data = recipe.to_dict()
    data["root_status_summary"] = _root_status_summary_for_recipe(recipe.id)
    data["has_load_error"] = _recipe_has_load_error(recipe.id)
...
async def _uniquify_recipe_name(
    session: AsyncSession,
    base_name: str,
    project_id: Optional[int],
) -> str:
    """Return `base_name` with a numeric suffix (" 1", " 2", …) appended until
    no non-deleted recipe in the same project scope shares the name.
    ...
    stmt = select(Recipe.name).where(
        Recipe.deleted_at.is_(None),
        Recipe.name.like(f"{stem}%"),
    )
    if project_id is None:
        stmt = stmt.where(Recipe.project_id.is_(None))
    else:
        stmt = stmt.where(Recipe.project_id == project_id)
...
def _build_metadata(recipe: Recipe) -> dict:
    return {
        ...
        "input_schema": json.loads(recipe.input_schema) if recipe.input_schema else None,
        "output_schema": json.loads(recipe.output_schema) if recipe.output_schema else None,
        "inputs": json.loads(recipe.inputs) if recipe.inputs else None,
        "execution_state": recipe.execution_state,
...
async def create_recipe(
    ...
    session.add(recipe)
    await session.flush()  # assign recipe.id before we create the on-disk dir
    ...
    create_recipe_directory(recipe.id, metadata=_build_metadata(recipe))

    # Hash the (empty) program file we just wrote; stored for future store keying.
    program_path = get_recipe_program_path(recipe.id)
    recipe.program_hash = _program_hash(program_path.read_text())
...
async def list_recipes(
    ...
    if not include_deleted:
        conditions.append(Recipe.deleted_at.is_(None))
    if execution_state is not None:
        ...
        conditions.append(Recipe.execution_state == execution_state)
    if project_id is not None:
        conditions.append(Recipe.project_id == project_id)
    else:
        conditions.append(Recipe.project_id.is_(None))
    if parent_id is not None:
        conditions.append(Recipe.parent_id == parent_id)

    query = select(Recipe)
    if conditions:
        query = query.where(and_(*conditions))
    query = query.order_by(desc(Recipe.updated_at), desc(Recipe.id))
...
async def update_recipe(
    ...
    if request.name is not None:
        recipe.name = request.name.strip()
    if request.description is not None:
        recipe.description = request.description
    if "project_id" in request.model_fields_set:
        if request.project_id is not None:
            await get_project_or_404(session, request.project_id)
        recipe.project_id = request.project_id
    if request.input_schema is not None:
        recipe.input_schema = json.dumps(request.input_schema)
    if request.output_schema is not None:
        recipe.output_schema = json.dumps(request.output_schema)
    if request.inputs is not None:
        recipe.inputs = json.dumps(request.inputs)
    if request.execution_state is not None:
        ...
        recipe.execution_state = request.execution_state

    recipe.updated_at = datetime.utcnow()
    await session.commit()
    await session.refresh(recipe)

    # If the user just changed the recipe's `inputs` and a runtime is live,
    ...
    if request.inputs is not None:
        runtime = recipe_registry.get_runtime(recipe_id)
        if runtime is not None:
            new_inputs = json.loads(recipe.inputs) if recipe.inputs else {}
            if runtime.inputs != new_inputs:
                runtime.inputs = dict(new_inputs)
                try:
                    runtime.try_reload_program()
                except Exception:  # noqa: BLE001 — best-effort
                    log.exception("inputs-change reload failed", extra={"recipe_id": recipe_id})
        # Auto-start if the user submitted inputs on an idle recipe — filling
        # the form is the "run this now" gesture, no separate play click.
        if recipe.execution_state == "idle":
            try:
                ...
                log.exception("auto-start on inputs PATCH failed", extra={"recipe_id": recipe_id})

    response = _serialize(recipe)
    await ws_manager.broadcast("recipe_updated", {"recipe": response.model_dump()})
    return response
...
async def delete_recipe(recipe_id: int, session: AsyncSession = Depends(get_db_session)):
    ...
    recipe.updated_at = deleted_at
    recipe.execution_state = "idle"
    await session.commit()
    # Stop any in-memory scheduler so a deleted recipe can't keep firing
...
async def restore_recipe(recipe_id: int, session: AsyncSession = Depends(get_db_session)):
    ...
    await session.refresh(recipe)
    response = _serialize(recipe)
    await ws_manager.broadcast("recipe_restored", {"recipe": response.model_dump()})
    return response
...
async def fork_recipe(
    recipe_id: int,
    request: RecipeForkRequest,
    session: AsyncSession = Depends(get_db_session),
):
    """Fork a recipe (primary "Run" operation).

    Creates a new recipe row with parent_id set, copies the on-disk directory
    ...
    """
    parent = await get_recipe_or_404(session, recipe_id)
    ...
    name = await _uniquify_recipe_name(session, base_name, project_id)
    ...
    parent_dir = get_recipe_dir(parent.id)
    if not parent_dir.exists():
        ...
        raise HTTPException(
            status_code=500,
            detail=f"parent recipe {parent.id} directory missing; cannot fork",
        )

    fork_recipe_directory(parent.id, fork.id, metadata=_build_metadata(fork))
...
# ---------------------------------------------------------------------------
# Phase 4 — Per-recipe tasks listing
# ---------------------------------------------------------------------------
...
@router.get("/{recipe_id}/tasks", response_model=list[RecipeTaskResponse])
async def list_recipe_tasks(
    recipe_id: int,
...
async def _set_execution_state(
    session: AsyncSession, recipe: Recipe, new_state: str,
) -> RecipeResponse:
    recipe.execution_state = new_state
    recipe.updated_at = datetime.utcnow()
    await session.commit()
    await session.refresh(recipe)
    response = _serialize(recipe)
...
@router.post("/{recipe_id}/start", response_model=RecipeResponse)
async def start_recipe(
    recipe_id: int,
    dry_run: bool = Query(False),
    session: AsyncSession = Depends(get_db_session),
):
    """Begin execution. Idempotent — already-running recipes return OK.

    Builds the graph from program.py (registering the runtime in
    `recipe_registry`), then starts the FRP scheduler.
    """
    recipe = await get_recipe_or_404(session, recipe_id)
    try:
        runtime = recipe_lifecycle.get_or_create_runtime(recipe)
        ...
        await ws_manager.broadcast("recipe_updated", {
            "recipe": _serialize(recipe).model_dump(),
    ...
    try:
        if dry_run:
            report = await dry_run_recipe(
                recipe_id=recipe.id,
                program_path=get_recipe_program_path(recipe.id),
                inputs=json.loads(recipe.inputs) if recipe.inputs else {},
                project_id=recipe.project_id,
            )
            if not report.ok:
                raise HTTPException(
                    status_code=400,
                    detail={"message": "recipe dry-run failed", "dry_run": report.to_dict()},
                )
        await runtime.start()
...
@router.post("/{recipe_id}/dry-run")
async def dry_run_recipe_route(
    recipe_id: int,
...
@router.post("/{recipe_id}/pause", response_model=RecipeResponse)
async def pause_recipe(
...
@router.post("/{recipe_id}/resume", response_model=RecipeResponse)
async def resume_recipe(
    recipe_id: int, session: AsyncSession = Depends(get_db_session),
):
    recipe = await get_recipe_or_404(session, recipe_id)
    runtime = recipe_lifecycle.get_or_create_runtime(recipe)
...
@router.post("/{recipe_id}/clear", response_model=RecipeResponse)
async def clear_recipe_runtime_state(
    recipe_id: int, session: AsyncSession = Depends(get_db_session),
):
    """Reset a recipe back to an empty program and empty compiled state."""
    recipe = await get_recipe_or_404(session, recipe_id)
    await recipe_lifecycle.stop_and_unregister(recipe_id)
    write_empty_recipe_program(recipe_id)
    graph_db.clear_recipe_state(get_recipe_state_db_path(recipe_id))
    recipe.program_hash = _program_hash(get_empty_recipe_program())
    recipe.input_schema = None
    recipe.execution_state = "idle"
    recipe.updated_at = datetime.utcnow()
    await session.commit()
    await session.refresh(recipe)
...
@router.post("/{recipe_id}/reparse", response_model=RecipeResponse)
async def reparse_recipe(
    recipe_id: int, session: AsyncSession = Depends(get_db_session),
):
    """Force a rebuild/reload of the recipe graph from program.py."""
    recipe = await get_recipe_or_404(session, recipe_id)
    await recipe_lifecycle.apply_program_edit(session, recipe_id)
    await session.refresh(recipe)
    return _serialize(recipe)
...
from recipe_runtime.summary import (
...
from recipe_runtime.step_timeline import build_phase_steps
...
def _humanize_input_name(name: Optional[str]) -> str:
    """Fallback when a recipe input has no explicit display_name.

    ``product_image`` → ``Product image``. Matches RecipeInputForm.vue's
...
def _definition_from_runtime(recipe_id: int, equation_key: str) -> Optional[dict[str, Any]]:
    runtime = recipe_registry.get_runtime(recipe_id)
...
def _resolve_live_dynamic_value(recipe_id: int, value: Any) -> Any:
    """Resolve NodeRefs from the live recipe graph for lightweight previews."""
    try:
    ...
    if isinstance(value, list):
        return [_resolve_live_dynamic_value(recipe_id, item) for item in value]
    if isinstance(value, tuple):
        return tuple(_resolve_live_dynamic_value(recipe_id, item) for item in value)
    if isinstance(value, dict):
        return {k: _resolve_live_dynamic_value(recipe_id, v) for k, v in value.items()}
    return value
...
def _render_prompt_preview(
    ...
    if not prompt_template:
        return ""
    if recipe_id is None or not prompt_dynamic:
        return prompt_template

    bindings = _resolve_live_dynamic_value(recipe_id, prompt_dynamic) or {}
...
def _output_name_by_key_for_recipe(recipe_id: int) -> dict[str, str]:
    """Equation keys surfaced by the recipe's return, keyed to output name.
...
def _output_type_by_key_for_recipe(recipe_id: int) -> dict[str, str]:
    ...
    *,
    ...
    if equation_type == "tool_call" and definition:
        ...
        if isinstance(raw_tool_id, str) and raw_tool_id:
            tool_id = raw_tool_id
        # task_type is required at the DSL layer (recipe_dsl.primitives.tool()
    ...
    if equation_type == "recipe_input" and definition:
    ...
    result_value = _json_safe_recipe_value(data.get("result"))
...
def _load_equations_for_recipe(recipe_id: int) -> list[dict[str, Any]]:
    """Load equation rows from state.db, parsing JSON columns.

    Falls back to an empty list when the state.db doesn't exist yet (a
    freshly-created recipe whose program.py has never been run).
    """
    db_path = get_recipe_state_db_path(recipe_id)
...
def _apply_runtime_overlay(
    rows: list[dict[str, Any]], recipe_id: int,
) -> list[dict[str, Any]]:
...
def _phase_tree_from_equations(
    ...
    def new_node(name: str, path: list[str]) -> RecipePhaseNodeResponse:
        return RecipePhaseNodeResponse(
            name=name,
            path=path,
            children=[],
            equation_keys=[],
    ...
    index: dict[tuple[str, ...], RecipePhaseNodeResponse] = {(): root}
    ...
    def ensure_node(path: list[str]) -> RecipePhaseNodeResponse:
        key = tuple(path)
        if key in index:
            return index[key]
    ...
    def collect_subtree_equation_keys(node: RecipePhaseNodeResponse) -> set[str]:
    ...
    def sort_children_by_dependencies(node: RecipePhaseNodeResponse) -> None:
    ...
    def rollup(node: RecipePhaseNodeResponse) -> tuple[dict[str, int], int]:
    ...
    rollup(root)
    if steps_by_phase:
        def attach_steps(node: RecipePhaseNodeResponse) -> None:
...
def _pending_task_counts_by_equation(recipe_id: int) -> dict[str, int]:
...
def _recipe_program_can_inflate(recipe_id: int) -> bool:
    """Return True when program.py has real recipe code to build."""
    ...
    try:
        ...
        return False
    return bool(program_text.strip()) and not is_empty_recipe_program(program_text)


def _inflate_runtime_for_read(recipe: Recipe):
    ...
    if runtime is not None:
        return runtime
    if not _recipe_program_can_inflate(recipe.id):
        return None
    try:
        return recipe_lifecycle.get_or_create_runtime(recipe)
    except Exception:  # noqa: BLE001 — read paths should still serve saved state
        log.exception("read-path runtime inflation failed", extra={"recipe_id": recipe.id})
        return None


async def _recover_running_runtime_for_read(recipe: Recipe):
    runtime = _inflate_runtime_for_read(recipe)
    if runtime is None:
        return None
    if recipe.execution_state == "running":
        await runtime.start()
    runtime.recover_orphaned_work()
    return runtime


@router.get("/{recipe_id}/phases", response_model=RecipePhaseTreeResponse)
async def get_phase_tree(
    ...
    if (
        not _load_equations_for_recipe(recipe_id)
        and recipe_lifecycle.get_cached_load_error(recipe_id) is None
    ):
        program_path = get_recipe_program_path(recipe_id)
        if program_path.exists() and program_path.stat().st_size > 0:
            try:
                ...
                program_text = ""
            if is_empty_recipe_program(program_text):
                recipe_lifecycle.clear_cached_load_error(recipe_id)
            else:
                try:
                    await recipe_lifecycle.apply_program_edit(session, recipe_id)
                except Exception:  # noqa: BLE001 — never let lazy-build 500 the read
                    log.exception("lazy apply_program_edit failed", extra={"recipe_id": recipe_id})

    await _recover_running_runtime_for_read(recipe)

    rows = _apply_runtime_overlay(_load_equations_for_recipe(recipe_id), recipe_id)
    task_counts = _pending_task_counts_by_equation(recipe_id)
    output_name_by_key = _output_name_by_key_for_recipe(recipe_id)
    output_type_by_key = _output_type_by_key_for_recipe(recipe_id)
    equation_responses = [
        _equation_to_response(
            r,
            definition=_definition_from_runtime(recipe_id, r["equation_key"]),
            recipe_id=recipe_id,
    ...
    if runtime is not None:
        ...
        program_path = get_recipe_program_path(recipe_id)
        try:
            is_empty_program = program_path.exists() and is_empty_recipe_program(program_path.read_text())
        except OSError:
            is_empty_program = False
        load_error_payload = None if is_empty_program else recipe_lifecycle.get_cached_load_error(recipe_id)

    return RecipePhaseTreeResponse(
        recipe_id=recipe.id,
        execution_state=recipe.execution_state,
        root=root,
...
@router.get("/{recipe_id}/program")
async def get_recipe_program(
    recipe_id: int, session: AsyncSession = Depends(get_db_session),
):
    await get_recipe_or_404(session, recipe_id)
    program_path = get_recipe_program_path(recipe_id)
    if not program_path.exists():
        return {"code": ""}
    try:
...
@router.put("/{recipe_id}/program")
async def update_recipe_program(
    recipe_id: int,
    body: _UpdateProgramRequest,
    session: AsyncSession = Depends(get_db_session),
):
    """Persist a user-authored program.py edit and rebuild the recipe graph.
    ...
    mark_user_program_edit(recipe_id)
    load_error = await recipe_lifecycle.apply_program_edit(session, recipe_id)
    return {
        "code": body.code,
        "load_error": load_error,
    }
...
@router.get("/{recipe_id}/equations", response_model=list[RecipeEquationResponse])
...
@router.get("/{recipe_id}/equations/{equation_key:path}/trace")
async def get_equation_trace(
    ...
    idle recipe we return the template with placeholders unrendered.
    """
    recipe = await get_recipe_or_404(session, recipe_id)
    runtime = _inflate_runtime_for_read(recipe)
    state_db_path = get_recipe_state_db_path(recipe_id)
    persisted_row = next(
        (row for row in _load_equations_for_recipe(recipe_id) if row["equation_key"] == equation_key),
        None,
    )
    if runtime is None:
        runtime = recipe_registry.get_runtime(recipe_id)
    ...
    if eq is None and persisted_row is None:
        raise HTTPException(
            status_code=404,
            detail=f"equation {equation_key!r} not found in recipe {recipe_id}",
        )
    ...
    return _json_safe_recipe_value(out)
...
@router.get(
    "/{recipe_id}/equations/{equation_key:path}",
    response_model=RecipeEquationResponse,
)
    ...
    for row in rows:
        if row["equation_key"] == equation_key:
            return _equation_to_response(
                row,
                definition=_definition_from_runtime(recipe_id, equation_key),
                recipe_id=recipe_id,
                output_name_by_key=_output_name_by_key_for_recipe(recipe_id),
                output_type_by_key=_output_type_by_key_for_recipe(recipe_id),
            )
...
@router.post(
    "/{recipe_id}/equations/{equation_key:path}/invalidate",
    response_model=dict,
)
async def invalidate_equation(
    ...
    Requires an active runtime; callers should start the recipe first (the
    ...
    if runtime.run is None:
        if recipe.execution_state != "running":
            raise HTTPException(
                status_code=409,
                detail="recipe is not running; call /start before invalidating",
            )
        await runtime.start()
        await _set_execution_state(session, recipe, "running")
    ...
    return {"recipe_id": recipe_id, "equation_key": equation_key, "reset": reset}


@router.post(
    "/{recipe_id}/equations/{equation_key:path}/reselect",
    response_model=dict,
)
async def reselect_equation(
    ...
    if runtime.run is None:
        if recipe.execution_state != "running":
            raise HTTPException(
                status_code=409,
                detail="recipe is not running; call /start before reselecting",
            )
        await runtime.start()
        await _set_execution_state(session, recipe, "running")
    if runtime.graph is None or runtime.graph.try_get(equation_key) is None:
    ...
    try:
        ...
        from recipe_runtime.evaluators import EvaluatorError
...
@router.post("/{recipe_id}/phases/invalidate", response_model=dict)
async def invalidate_phase(
    ...
    if not matching:
        return {"recipe_id": recipe_id, "phase_path": prefix, "reset": 0, "matched": 0}
