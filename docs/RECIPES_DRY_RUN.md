...
Recipe dry-run is a fast preflight execution path for catching structural
recipe errors before real provider calls run.

It uses the real graph builder and `RecipeRun` scheduler against a temporary
...
1. **Automatically on every recipe parse / reparse / update** — dry-run is
   invoked from `recipe_lifecycle.apply_program_edit()` after a successful
   graph build, so deferred-callback errors surface in the agent's edit
   feedback (and the `recipe_updated` `load_error` payload) before the user
...
2. Explicit endpoints for ad-hoc preflight:
   - `POST /api/recipes/{recipe_id}/dry-run`
   - `POST /api/recipes/{recipe_id}/start?dry_run=true` — when enabled and
