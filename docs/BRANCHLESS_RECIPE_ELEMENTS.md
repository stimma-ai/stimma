# Branchless Recipe Elements

This note describes the current design direction for conditional behavior in
Stimma recipes after removing `branch()`. The goal is an equation graph that is
...
Recipes are functional-reactive equation graphs. A recipe program builds a graph
