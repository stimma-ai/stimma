...
- **Official builds only.** The telemetry client is a permanent no-op
  ...
  in the source tree, but non-official builds do not buffer or transmit.
...
| `telemetry_enabled` | live | `{ enabled: true }` — fired when telemetry is turned on |
...
consent; pre-consent buffering semantics; Privacy Lockdown kills telemetry
and suppresses Stimma service contact; the User-Agent emits no install ID
under lockdown; the telemetry body carries no install id. `backend/tests/test_model_family.py`
pins the munger fixtures and the raw-string-never-egresses property.
