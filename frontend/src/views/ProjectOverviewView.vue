<template>
  <div class="relative h-full flex flex-col bg-base">
    ...
    <div v-else class="h-full overflow-y-auto custom-scrollbar">
      <div class="min-h-full flex flex-col items-center px-8 lg:px-16 transition-opacity duration-150 ease-in" :class="loaded ? 'opacity-100' : 'opacity-0'">
        ...
        <div class="relative flex-1 flex flex-col items-center justify-center w-full pt-24 pb-16">
          ...
          <h1 class="relative font-brand text-[32px] font-bold tracking-tight text-content mb-2 text-center">{{ heroParts.pre }}<span class="bg-gradient-to-br from-teal-400 via-cyan-400 to-indigo-400 bg-clip-text text-transparent">{{ heroParts.word }}</span>{{ heroParts.post }}</h1>
          <!-- Spacer where the home greeting subtitle sits — keeps the hero rhythm in sync -->
          ...
          <div class="relative w-full max-w-[720px]">
            ...
            <div v-if="launcherTools.length > 0" class="flex flex-wrap justify-center gap-2 mt-4">
              <button
        ...
        <div ref="contentRef" class="w-full max-w-[960px] pb-12 space-y-10">
          ...
          <div v-if="recentMedia.length > 0">
            <div class="flex items-center justify-between mb-3">
              <h2 class="text-xs font-semibold text-content-secondary">Fresh in this project</h2>
...
// mirroring the home greeting — here it lands on the project name.
const heroParts = computed(() => {
  const name = props.project?.name?.trim()
  if (!name) return { pre: 'What would you like to ', word: 'create', post: ' today?' }
  return { pre: 'How can I help with ', word: name, post: '?' }
})
...
// take a slot. Same treatment as the home page — tools aren't project-scoped.
const launcherTools = computed(() => {
  ...
  for (const pick of pickStarterTools(availableTools.value, 4)) {
    if (picked.length >= 3) break
    tryAdd(pick.tool)
  }
  return picked
...
// sync) — often after this screen has already rendered. Reload tools on
// provider/tool change events so the launcher pills fill in as tools exist.
