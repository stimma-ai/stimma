<template>
  <div class="h-full flex flex-col bg-base" @contextmenu.self="handleEmptyContextMenu">
    ...
    >
      ...
      <!-- Empty state -->
      <div v-else-if="filteredChats.length === 0 && chats.length === 0" class="flex flex-col items-center justify-center h-64 compact:h-full text-center">
    ...
    <!-- Entity Context Menu -->
    <RenameSheet :show="renameTarget !== null" :name="renameTarget?.name || ''" label="Rename chat" @close="renameTarget = null" @save="renameFromSheet" />
    <EntityContextMenu
...
async function handleContextMenuRename(entityType, entityId, entityName) {
  // Compact: rename in place from the row's sheet. Desktop: the chat opens in edit.
  if (isCompact.value) { renameTarget.value = chats.value.find((c) => c.id === entityId) || { id: entityId, name: entityName }; return }
  router.push({ name: 'chat', params: { id: entityId }, query: { rename: '1' } })
}

const renameTarget = ref(null)
async function renameFromSheet(name) {
  const chat = renameTarget.value
