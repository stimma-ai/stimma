      ...
      <div class="flex items-center gap-3 min-w-0 flex-shrink">
        ...
        <div v-if="copies.length > 0" class="relative flex-shrink-0" ref="copiesRootEl">
          <button
            ...
            @click="toggleCopies"
            :title="copies.length === 1 ? '1 copy of this flow' : `${copies.length} copies of this flow`"
          >
      ...
      <div class="flex items-center gap-1.5 flex-shrink-0 justify-end">
        ...
        <div class="relative" ref="chatMenuContainerRef">
          ...
          >
            ...
            <div v-else-if="flowChatSessions.length === 0" class="px-3 py-2 text-xs text-content-muted italic">
              No past sessions.
            </div>
            <div v-else class="max-h-64 overflow-y-auto">
              <button
                v-for="session in flowChatSessions"
                :key="session.id"
    ...
    <div class="flex flex-1 min-h-0">
      ...
      <div class="flex-1 flex flex-col min-w-0">
          ...
          <div class="ml-auto flex items-center gap-2 py-1">
            ...
            <div class="relative" ref="tabMenuContainerRef">
              ...
              >
                ...
                <button
                  @click="handleReparseFlow"
                  :disabled="reparsingFlow"
                  class="w-full px-3 py-1.5 text-left text-xs text-content-secondary hover:bg-surface-raised transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h-3.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992V4.356m-1.636 14.288A9 9 0 1 1 21 12.75v-3" />
                  </svg>
                  {{ reparsingFlow ? 'Re-processing…' : 'Re-process flow' }}
                </button>
                <button
                  @click="handleClearFlow"
                  :disabled="clearingFlow"
                ...
                >
                  ...
                  {{ clearingFlow ? 'Clearing…' : 'Clear flow' }}
                </button>
              </div>
            </div>
          </div>
        ...
        <FlowAttentionBanner
          ...
          :dev-mode="isDev"
          :can-pause="flow?.execution_state === 'running'"
          ...
          <div class="flex-1 flex flex-col min-w-0">
              ...
              <div class="space-y-4">
                ...
                >
                  <div class="max-w-md text-center">
                    ...
                    <div class="text-[15px] font-semibold text-content">No flow yet</div>
                      ...
                      <div class="py-1">
                        ...
                        <template v-else>
                          <PhaseTree
                            ...
                            :execution-state="flow?.execution_state || 'idle'"
                            :dev-mode="isDev"
            ...
            <div v-else-if="viewMode === 'graph'" class="flex-1 min-h-0 relative flex flex-col">
              <div class="relative flex-1 min-h-0">
                <div
                  v-if="showFlowEmptyState && !programAttentionError"
                ...
                <EquationGraph
                  ...
                  :flow-id="flowId"
                  :input-schema="flow?.input_schema || null"
                  :input-values="flow?.inputs || null"
            ...
            <div v-else-if="viewMode === 'code'" class="flex-1 min-h-0 relative flex flex-col">
              ...
              >
                ...
                <span v-if="savingCode" class="text-[11px] text-content-muted">Re-processing flow…</span>
          ...
          <div v-else-if="chatError" class="flex-1 flex items-center justify-center px-4 text-center">
            <div class="space-y-2">
              ...
              <button
                ...
                @click="ensureFlowChat"
              >Retry</button>
            </div>
          </div>
          <ChatView
            v-else-if="flowChatId != null"
            :key="flowChatId"
            :chat-id="flowChatId"
            :embedded="true"
            class="flex-1 min-h-0"
...
import PhaseTree from '../components/flow/PhaseTree.vue'
import DeliveryPanel from '../components/flow/DeliveryPanel.vue'
import EquationGraph from '../components/flow/EquationGraph.vue'
import FlowAttentionBanner from '../components/flow/FlowAttentionBanner.vue'
import FlowCodeView from '../components/flow/FlowCodeView.vue'
...
import { useFlowCodeErrors } from '../composables/useFlowCodeErrors'
import { useFlowsApi, type Flow, type FlowTask, type FlowEquation, type PhaseNode as PhaseNodeType } from '../composables/useFlowsApi'
...
function getOrderedPhaseEquations(phasePath: string[]): FlowEquation[] {
  ...
  return ordered.filter(isVisibleFlowEquation)
}

function extractTaskMediaIds(task: FlowTask): number[] {
...
function collectFlowMediaIds(): number[] {
  const root = state.phaseTree.value?.root
  if (!root) return []

  const tasksByEquationKey = new Map<string, FlowTask[]>()
...
const flowSlideshowMediaIds = computed(() => collectFlowMediaIds())
let stopFlowSlideshowListWatch: WatchStopHandle | null = null

function exitSlideshow(options?: { saveContext?: boolean }) {
  stopFlowSlideshowListWatch?.()
  stopFlowSlideshowListWatch = null
  exitBaseSlideshow(options)
}

function openFlowMediaSlideshow(mediaId: number, options?: { mediaIds?: number[]; startIndex?: number }) {
  ...
  const currentIds = computed(() => {
    ...
    const ids = flowSlideshowMediaIds.value.slice()
  ...
  stopFlowSlideshowListWatch?.()
  stopFlowSlideshowListWatch = watch(currentIds, (ids) => {
...
provide(flowMediaSlideshowKey, {
  open: openFlowMediaSlideshow,
})
...
function startEditName() {
  editingNameValue.value = flow.value?.name || ''
...
async function saveName() {
  ...
  if (name === (flow.value.name || '')) return
...
// flow via chat, or pause) to unstick it.
...
async function resolveParentName() {
  ...
  const pid = flow.value?.parent_id
  if (!pid) return
  try {
    const parent = await api.getFlow(pid)
    parentName.value = parent.name || `Flow #${pid}`
  } catch { parentName.value = `Flow #${pid}` }
}
watch(() => flow.value?.parent_id, () => resolveParentName(), { immediate: true })

function openParent() {
  if (flow.value?.parent_id) router.push({ name: 'flow', params: { id: String(flow.value.parent_id) } })
}

// --- Copies dropdown ---
// Flat list of child flows (parent_id == this flow). Loaded eagerly on
// flow change so we can hide the pill when there are zero copies, and
// refreshed on flow_created / flow_deleted WS events.
const copies = ref<Flow[]>([])
const copiesLoading = ref(false)
const copiesOpen = ref(false)
const copiesRootEl = ref<HTMLElement | null>(null)

async function loadCopies() {
  if (!flow.value) { copies.value = []; return }
  copiesLoading.value = true
  try {
    copies.value = await api.listFlows({ parent_id: flow.value.id })
  } catch { copies.value = [] }
...
function openCopy(c: Flow) {
  copiesOpen.value = false
  router.push({ name: 'flow', params: { id: String(c.id) } })
}
...
async function handleReparseFlow() {
  showMenu.value = false
  showTabMenu.value = false
  await doReparseFlow()
}
...
type FlowChatSession = { id: number; name: string; created_at: string; updated_at: string }

const showChatMenu = ref(false)
const chatMenuContainerRef = ref<HTMLElement | null>(null)
const flowChatSessions = ref<FlowChatSession[]>([])
...
async function loadChatSessions() {
  ...
  try {
    ...
    const response = await axios.get(`${base}/chats`, {
      params: { flow_id: flow.value.id, page: 1, page_size: 50 }
    })
    flowChatSessions.value = (response.data?.items || []) as FlowChatSession[]
  } catch (err) {
...
async function handleNewChatSession() {
  ...
  try {
    const base = getApiBase()
    const created = await axios.post(`${base}/chats`, { flow_id: flow.value.id })
    const newId = created.data?.id
    if (newId) {
      flowChatId.value = Number(newId)
...
function handleSwitchChatSession(chatId: number) {
  if (chatId !== flowChatId.value) {
    flowChatId.value = chatId
    chatPanelOpen.value = true
  }
  showChatMenu.value = false
}
...
function sessionDisplayName(session: FlowChatSession): string {
...
async function doPlay() {
  try {
    if (flow.value?.execution_state === 'paused') {
      await state.resume()
    } else {
      await state.start()
    }
...
// --- Task resolution ---
async function onResolveTask(task: FlowTask, resolution: any) {
  try {
    await state.resolveTask(task.task_id, resolution)
  } catch (err) { console.error('resolve failed', err) }
}
async function onResolveErrorTask(task: FlowTask, action: string, value?: any) {
  try {
...
// When the slideshow is open inside a flow, surface the approve task
...
const inputFormRef = ref<InstanceType<typeof FlowInputForm> | null>(null)
...
const showFlowEmptyState = computed(() => {
...
const failedRuntimeEquations = computed<FlowEquation[]>(() =>
...
const runtimeAttentionError = computed(() => {
  ...
  const parsed = parseFlowError(firstWithError.error)
  ...
  return {
    ...
    message: parsed?.message || `${stepName} failed. Ask the assistant to inspect the failed step and update the flow.`,
    suggestion: null,
...
const showFlowAttentionBanner = computed(() => !!attentionError.value || allBlocked.value)
...
// values map (one key for the selected flow_input node). Merge it onto the
// flow's existing inputs so the other fields keep their values, then write
// the whole blob back the same way the steps-view form does.
async function onPanelInputUpdate(partial: Record<string, any>) {
  if (!flow.value) return
  const merged = { ...(flow.value.inputs || {}), ...partial }
  submittingInputs.value = true
  try {
...
async function doCopy() {
  ...
  copying.value = true
  try {
    const copy = await state.fork(flow.value.inputs || {})
    router.push({ name: 'flow', params: { id: String(copy.id) } })
  } catch (err) {
...
const reparsingFlow = ref(false)
const clearingFlow = ref(false)
const codeViewRef = ref<InstanceType<typeof FlowCodeView> | null>(null)
const codeDirty = ref(false)
const savingCode = ref(false)
const programCodeDraft = ref('')
const showCodeIntroModal = ref(false)

const codeErrors = useFlowCodeErrors(state.programLoadError, state.equations)
...
// Drop a stale selection when the underlying error list changes (flow
...
function getProgramDraftKey(rid = flowId.value) {
  return rid ? makeStorageKey('flow', rid, 'program_draft') : null
}

function getCodeIntroKey() {
  return makeStorageKey('flow', 'code_intro_seen')
}

function getStoredProgramDraft(rid = flowId.value): string | null {
...
function clearStoredProgramDraft(rid = flowId.value) {
  const key = getProgramDraftKey(rid)
  if (!key) return
...
async function saveCodeEdit() {
  if (!flowId.value || !codeViewRef.value) return
  const next = codeViewRef.value.getCurrentCode()
  if (savingCode.value || next === programCode.value) return
  savingCode.value = true
  try {
    const result = await api.updateProgramCode(flowId.value, next)
    ...
    if (result.load_error) {
      const msg = result.load_error.message || 'Flow failed to compile'
...
async function doReparseFlow() {
  reparsingFlow.value = true
  try {
    await state.reparse()
    ...
    reparsingFlow.value = false
  }
}

async function doClearFlow() {
  clearingFlow.value = true
  try {
    await state.clear()
    ...
    clearingFlow.value = false
  }
}

// --- Phase tree view mode ---
type FlowViewMode = 'compact' | 'graph' | 'code'
const VIEW_MODE_KEY = 'flow.viewMode'
const _storedViewMode = localStorage.getItem(VIEW_MODE_KEY)
const viewMode = ref<FlowViewMode>(
...
const GRAPH_COLLAPSE_KEY = 'flow.graphCollapseForeach'
...
async function loadProgramCode() {
  ...
  const rid = flowId.value
  programCodeLoading.value = true
  try {
    const code = await api.getProgramCode(flowId.value)
...
watch(() => state.flow.value?.program_hash, () => {
...
function getChatPanelOpenKey(rid: string) {
  return makeStorageKey('flow', rid, 'chat_panel_open')
}

function getChatPanelWidthKey(rid: string) {
  return makeStorageKey('flow', rid, 'chat_panel_width')
}
...
const flowChatId = ref<number | null>(null)
...
provide(FLOW_CHAT_ID_KEY, flowChatId)

function toggleChatPanel() { chatPanelOpen.value = !chatPanelOpen.value }

async function ensureFlowChat() {
  if (!flow.value) return
  const rid = flow.value.id
  chatLoading.value = true
  chatError.value = false
  try {
    const base = getApiBase()
    const list = await axios.get(`${base}/chats`, { params: { flow_id: rid, page: 1, page_size: 1 } })
    let chatId = list.data?.items?.[0]?.id
    if (!chatId) {
      const created = await axios.post(`${base}/chats`, { flow_id: rid })
      chatId = created.data?.id
    }
    flowChatId.value = chatId ? Number(chatId) : null
    if (!flowChatId.value) chatError.value = true
  } catch (err) {
    console.error('Failed to open flow chat:', err)
...
// Re-resolve the chat whenever the flow id changes (initial load or switching flows).
watch(() => flow.value?.id, (rid) => {
  flowChatId.value = null
  chatError.value = false
  if (rid != null) ensureFlowChat()
}, { immediate: true })

function openChatForEdit(_task: FlowTask) {
  chatPanelOpen.value = true
}

// --- Ask-the-agent-to-fix flow ---
// Posts the current build error into the flow-scoped chat so the agent
...
// Agent-running signal for the flow-scoped chat. We watch WS events for
...
let offFlowCreated: (() => void) | null = null
let offFlowDeleted: (() => void) | null = null

async function syncFlowAgentStatus() {
  if (flowChatId.value == null) {
    agentRunningInChat.value = false
    return
  }
  try {
    const base = getApiBase()
    const response = await fetch(`${base}/chats/${flowChatId.value}/plan-status`)
    if (!response.ok) return
    const data = await response.json()
    agentRunningInChat.value = Boolean(data?.has_plan && data?.status === 'running' && !data?.is_stale)
  } catch (error) {
    console.error('Failed to sync flow agent status:', error)
  }
}

onMounted(() => {
  offAgentStarted = onWs('agent_started', (data: any) => {
    if (flowChatId.value != null && data?.chat_id === flowChatId.value) {
  ...
  // disappear. New copies are only added as children of the current flow;
  // deletes can affect this flow's children regardless of which view
  // triggered them, so reload unconditionally.
  offFlowCreated = onWs('flow_created', (data: any) => {
    const parentId = data?.flow?.parent_id
    if (flow.value && parentId === flow.value.id) loadCopies()
  })
  offFlowDeleted = onWs('flow_deleted', () => {
    if (flow.value) loadCopies()
  })
})
onUnmounted(() => {
  offAgentStarted?.()
...
// Switching flows (and thus chats) clears the running flag; the next
...
async function askAgentToFix(_loadError: { category: string; message: string; suggestion?: string | null }) {
  ...
  if (!flow.value) return
  if (flowChatId.value == null) {
    await ensureFlowChat()
    if (flowChatId.value == null) return
  }
  fixingWithAgent.value = true
  try {
    const base = getApiBase()
    // Deliberately short and jargon-free — the assistant sees the real build
    // error via analyze_flow, so we don't need to echo technical details
    // into the user-visible chat stream.
    const messageText = "This flow isn't building — please take a look and fix it."
    await axios.post(`${base}/chats/${flowChatId.value}/items`, {
...
async function askAgentForHelpUnblocking() {
  ...
  try {
    const base = getApiBase()
    // The assistant reads analyze_flow for the real errors; this message
    // just points it at the failed runtime steps.
    const messageText = 'Some flow steps failed. Please take a look and fix the flow.'
    await axios.post(`${base}/chats/${flowChatId.value}/items`, {
      item_type: 'user_message',
      message_text: messageText,
    })
...
async function askAgentToFixStep(equation: { display_name?: string | null; equation_key: string }) {
  ...
  try {
    const base = getApiBase()
    // Step-level fix request. The assistant reads analyze_flow for the
