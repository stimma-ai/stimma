...
export function parseFlowError(rawError: unknown): FlowErrorInfo | null {
...
export function errorSummary(rawError: unknown): string {
  const parsed = parseFlowError(rawError)
...
function userMessageForError(category: string | null, message: string): string {
  ...
  if (category === 'code_error') {
    return `${jobPrefix}the flow code for this step failed: ${core}`
  }
