...
export function resolveModelVendorId(model: ModelLike | string | null | undefined): ModelVendorId | null {
  ...
  if (typeof model === 'string') return VENDOR_ALIASES[model.toLowerCase()] || null
...
function modelOrder(model: ModelLike): number {
  ...
  if (!vendor) return 90
  return { anthropic: 9, openai: 19, xai: 20, minimax: 30, kimi: 40, alibaba: 50, stepfun: 60, zai: 70 }[vendor]
}
