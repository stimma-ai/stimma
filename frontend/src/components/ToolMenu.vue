<template>
  <div class="w-48 bg-surface border border-edge-subtle rounded-lg shadow-xl py-1">
    <!-- Pin/Unpin (always available) -->
    <button
      @click="$emit('toggle-pin')"
      class="w-full px-3 py-2 text-left text-sm text-content-secondary hover:text-content hover:bg-overlay-subtle transition-colors flex items-center gap-2"
    >
      <svg class="w-4 h-4" viewBox="0 0 24 24" :fill="pinned ? 'currentColor' : 'none'" :stroke="pinned ? 'none' : 'currentColor'" stroke-width="2">
    ...
    <template v-if="showResetDefaults">
