...
const cardClass = computed(() => {
  switch (props.status) {
    case 'completed':      return 'bg-flow-done-tint  border-flow-done-soft  hover:shadow-md'
    case 'computing':      return 'bg-flow-run-tint   border-flow-run-soft   hover:shadow-md'
    case 'failed':         return 'bg-flow-fail-tint  border-flow-fail-soft  hover:shadow-md'
    case 'awaiting_input': return 'bg-flow-await-tint border-flow-await-soft hover:shadow-md'
    case 'invalidated':    return 'bg-flow-stale-tint border-flow-stale-soft hover:shadow-md'
...
const flagClass = computed(() => {
  switch (props.status) {
    case 'completed':      return 'bg-flow-done-strong'
    case 'computing':      return 'flow-flag-running'
    case 'failed':         return 'bg-flow-fail-strong'
    case 'awaiting_input': return 'bg-flow-await-strong'
    case 'invalidated':    return 'bg-flow-stale-strong'
...
const dividerClass = computed(() => {
  switch (props.status) {
    case 'completed':      return 'border-flow-done-soft'
    case 'computing':      return 'border-flow-run-soft'
    case 'failed':         return 'border-flow-fail-soft'
    case 'awaiting_input': return 'border-flow-await-soft'
    case 'invalidated':    return 'border-flow-stale-soft'
...
const dotClass = computed(() => {
  switch (props.status) {
    case 'completed':      return 'bg-flow-done-strong'
    case 'computing':      return 'bg-flow-run-strong flow-dot-pulse'
    case 'failed':         return 'bg-flow-fail-strong'
    case 'awaiting_input': return props.actionable ? 'bg-flow-await-strong flow-dot-pulse' : 'bg-content-muted/40'
    case 'invalidated':    return 'bg-flow-stale-strong'
...
const labelClass = computed(() => {
  switch (props.status) {
    ...
    case 'awaiting_input': return props.actionable ? 'text-flow-await-strong' : 'text-content-muted/70'
...
.flow-flag-running {
  background: linear-gradient(
    180deg,
    var(--color-flow-run-strong) 0%,
    var(--color-flow-run-mid)    35%,
    var(--color-flow-run-strong) 65%,
    var(--color-flow-run-mid)    100%
  );
  background-size: 100% 240%;
  animation: flow-flag-shimmer 1.8s linear infinite;
}
