...
import FlowIterationRow from './FlowIterationRow.vue'
import FlowMediaTile from './FlowMediaTile.vue'
import FlowIterationModal from './FlowIterationModal.vue'
...
const EMPTY_TASK_MAP: Map<string, FlowTask[]> = new Map()

const flowIdRef = computed(() => props.flowId)
const { isExpanded, toggle } = useFlowExpandState(flowIdRef)
...
const groupIsEchoed = computed<boolean>(() =>
  !!groupRefKey.value && flowRefs.hoveredRefKey.value === groupRefKey.value,
)
...
// this produced" for the common case, and the flow's final outputs
...
// those iterations, stalling the flow behind a chevron.
...
const pendingApproveTasks = computed<FlowTask[]>(() => {
...
// name. task_type is required at the DSL layer (flow_dsl.primitives.tool()
...
// this flow view is designed for.
const INLINE_MAX = 12
