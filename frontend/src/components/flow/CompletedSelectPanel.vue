...
async function fetchTrace() {
  ...
  try {
    const url = `${getApiBase()}/flows/${props.flowId}/equations/${encodeURIComponent(props.equation.equation_key)}/trace`
...
function cacheKey(): string {
  return `${props.flowId ?? 'none'}:${props.equation.equation_key}`
}

watch(() => [props.flowId, props.equation.equation_key], () => {
  fetchTrace()
}, { immediate: true })
</script>
