...
import { mapJobStatus, textClass, dotClass, bgClass } from '../../utils/statusColors'
...
function placeholderClass(status?: string | null): string {
  if (!status || (status !== 'failed' && status !== 'processing')) {
    return 'bg-overlay-faint text-content-muted'
  }
  const bucket = mapJobStatus(status)
