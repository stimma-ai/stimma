<template>
  ...
  <div v-if="dismissed" class="relative mb-3 rounded-lg border border-edge-subtle bg-overlay-subtle overflow-hidden">
    <div class="flex items-center gap-3 px-3 py-2">
      ...
      <button
        @click="$emit('restore')"
        class="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-md border border-purple-500/40 text-purple-400 hover:text-purple-300 hover:bg-purple-500/10 transition-colors cursor-pointer bg-transparent whitespace-nowrap"
      >
      ...
      <button
        @click="$emit('dismiss')"
        class="p-0.5 text-content-muted hover:text-content transition-colors rounded hover:bg-overlay-light cursor-pointer"
        title="Dismiss"
      >
    ...
    <!-- Countdown bar -->
    <div class="absolute bottom-0 left-0 h-0.5 bg-purple-500/30 transition-all duration-1000 ease-linear" :style="{ width: countdownPercent + '%' }" />
  </div>

  <!-- Normal remix banner -->
  <div v-else class="relative mb-3 rounded-lg border border-edge-subtle bg-overlay-subtle">
    <div class="flex items-start gap-3 p-2.5">
      ...
      <div class="min-w-0 pt-1 pr-6 overflow-hidden" style="flex: 1 1 0%;">
        ...
        <div class="flex flex-wrap gap-1.5 mt-2">
          <button
            ...
            class="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-md border border-edge-subtle text-content-secondary hover:text-content hover:bg-overlay-light transition-colors cursor-pointer bg-transparent"
      ...
      <button
        @click="$emit('dismiss')"
        class="absolute top-2 right-2 p-1 text-content-muted hover:text-content transition-colors rounded hover:bg-overlay-light cursor-pointer"
        title="Dismiss"
      >
