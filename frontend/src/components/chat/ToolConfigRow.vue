<template>
  <div
    class="group px-3 py-2 hover:bg-overlay-subtle transition-colors"
    :class="showBorder ? 'border-b border-edge-subtle' : ''"
  >
      ...
      <div class="flex items-center gap-1 flex-shrink-0">
        ...
        <div class="flex items-center gap-0.5 bg-overlay-subtle rounded-md p-0.5">
          <button
            ...
            :class="[
              'h-6 px-2 flex items-center justify-center rounded-md transition-colors text-[11px] font-medium',
              currentState === 'denied'
                ? [bgClass('failed'), textClass('failed')]
          ...
          <button
            ...
            :class="[
              ...
              currentState === 'neutral'
                ? [bgClass('queued'), 'text-content-secondary']
                : 'text-content-muted hover:text-content-tertiary hover:bg-overlay-subtle'
            ]"
        ...
        <Tooltip text="Remove from list">
          <button
            @click="$emit('remove')"
            class="w-6 h-6 flex items-center justify-center rounded-md text-content-muted hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 ring-accent/60"
          >
