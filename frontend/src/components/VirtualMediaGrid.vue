        ...
        <div v-else class="overflow-visible box-border w-full min-w-0" :style="rowStyle">
          <div class="grid min-w-0" :style="gridStyle">
            ...
            >
              ...
              <div v-if="getMediaType(rowItem) !== 'image' && getMediaType(rowItem) !== 'text'" class="absolute top-2 right-2 z-chrome">
                <div class="bg-black/60 backdrop-blur-md rounded-md px-1.5 py-1 flex items-center gap-1">
                  ...
                  <!-- Package icon (archive box: a package, not a cube; monochrome like vector/sprite) -->
                  <svg v-else-if="getMediaType(rowItem) === 'package'" class="w-4 h-4 flex-shrink-0 text-zinc-300" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
...
function handleAutoDeleteRemoved(data) {
  ...
  if (props.mediaList) {
    const projected = [...itemsCache.value.values()].find(item => mediaIdOf(item) === media_id)
    const identity = assetIdOf(projected || {})
    if (identity) props.mediaList.updateItem(identity, { expires_at: null })
  } else {
    ...
    for (const [index, item] of newCache.entries()) {
      if (item && mediaIdOf(item) === media_id) {
        newCache.set(index, { ...item, expires_at: null })
        break
      }
    }
    localItemsCache.value = newCache
