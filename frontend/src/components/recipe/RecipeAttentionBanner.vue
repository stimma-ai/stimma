    ...
    <div class="flex items-start gap-2">
      ...
      <div class="flex-1 min-w-0">
        <template v-if="agentBusy">
          <div class="font-semibold">The assistant is working on this recipe.</div>
...
const title = computed(() => {
  ...
  if (props.blocked && !props.error) return 'This recipe is blocked by errors.'
  return 'This recipe has build errors.'
})

const body = computed(() => {
  if (props.error && props.errorSource === 'runtime') {
    return props.error.message || 'Ask the assistant to inspect the failed step and update the recipe.'
  }
  if (props.blocked && !props.error) {
    return "It can't continue until something changes. Ask the assistant for help or pause the recipe."
  }
  return "It can't run until the recipe is fixed."
})
</script>
