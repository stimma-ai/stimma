...
import type { StepItem } from '../../composables/useRecipeGrouping'
...
// Top-level phases (direct children of the recipe root) render as cards with
...
const phaseIsEchoed = computed<boolean>(() =>
  recipeRefs.hoveredRefKey.value === phaseRefKey.value,
)
function isEqEchoed(eq: RecipeEquation): boolean {
  return recipeRefs.hoveredRefKey.value === eq.equation_key
}
// Label for an equation row's breadcrumb — the immediate parent phase name.
function eqBreadcrumb(eq: RecipeEquation): string {
...
function isCompositeEquation(eq: RecipeEquation): boolean {
  return eq.equation_type === 'create_grid' || eq.equation_type === 'create_set'
}

const displayName = computed(() => props.node.name || 'Recipe')
...
//   5. done
// Vocabulary mirrors useRecipeStatus: Error > Your Turn > Running > Waiting > Done.
...
// scaffolding (recipe_input and control wrappers) so the subtitle reflects
// real user-visible work, not plumbing.
function collectSubtreeEquations(n: PhaseNodeType): RecipeEquation[] {
  const out: RecipeEquation[] = []
  for (const key of n.equation_keys || []) {
    const eq = props.equationsByKey.get(key)
...
function orderedVisibleEquationsForPhase(path: string[]): RecipeEquation[] {
  ...
  const orderedAll: RecipeEquation[] = []
...
const directEquations = computed<RecipeEquation[]>(() => {
...
function infoMarkdown(eq: RecipeEquation): string {
  const r = eq.result
...
const directEquationsOrdered = computed<RecipeEquation[]>(() =>
  orderedVisibleEquationsForPhase(props.node.path || []),
)

const orderedSteps = computed<RecipeEquation[]>(() => directEquationsOrdered.value)
...
// expanded rows silently re-collapsed when the agent touched the recipe).
const expandState = useRecipeExpandState(recipeId)
...
function infoDefaultExpanded(eq: RecipeEquation): boolean {
  return eq.status !== 'pending'
}
function isInfoExpanded(eq: RecipeEquation): boolean {
  return expandState.isExpanded('info', eq.equation_key, infoDefaultExpanded(eq))
}
function toggleInfoExpanded(eq: RecipeEquation) {
  expandState.toggle('info', eq.equation_key, isInfoExpanded(eq))
}
function infoRowBorderClass(eq: RecipeEquation): string {
...
function equationStatusLabel(eq: RecipeEquation): string | null {
  switch (eq.status) {
...
function equationStatusColor(eq: RecipeEquation): string {
  switch (eq.status) {
...
function equationDurationLabel(eq: RecipeEquation): string | null {
  const ms = equationDurationMs(eq)
  return formatEquationDurationMs(ms)
}

function infoRowBgClass(eq: RecipeEquation): string {
  if (eq.status === 'failed') return 'bg-red-500/5'
  if (eq.status === 'computing') return 'bg-teal-500/5'
  return ''
}
function infoExpandedBorderClass(eq: RecipeEquation): string {
  if (eq.status === 'failed') return 'border-red-500/20'
  if (eq.status === 'computing') return 'border-teal-500/20'
  return 'border-edge-subtle'
}

function getTasksForEquation(key: string): RecipeTask[] {
...
// is noise.
function hitlDefaultExpanded(eq: RecipeEquation): boolean {
...
// disappear behind a chevron and stall the recipe. Same condition that
// produces the "Your Turn" label, so lock and label move together.
function isHitlLockedOpen(eq: RecipeEquation): boolean {
  return eq.status !== 'completed' && isEquationActionable(eq)
}
function isHitlExpanded(eq: RecipeEquation): boolean {
  if (isHitlLockedOpen(eq)) return true
  return expandState.isExpanded('hitl', eq.equation_key, hitlDefaultExpanded(eq))
}
function toggleHitlExpanded(eq: RecipeEquation) {
...
function hitlTitle(eq: RecipeEquation): string {
...
function hitlSubtitle(eq: RecipeEquation): string | null {
  if (eq.status === 'completed') return null
  if (!isEquationActionable(eq)) return null
  return eq.display_name || hitlKindLabel(eq.hitl_kind)
}

function hitlRowBorderClass(eq: RecipeEquation): string {
  if (eq.status === 'completed') return 'border-edge-subtle bg-overlay-subtle/30'
...
function hitlRowBgClass(eq: RecipeEquation): string {
  if (isEquationActionable(eq) && getTasksForEquation(eq.equation_key).length > 0) {
...
function hitlExpandedBorderClass(eq: RecipeEquation): string {
...
function completedHitlLabel(eq: RecipeEquation): string {
