...
import { equationIsReadyToSchedule } from '../../composables/useRecipeGrouping'
...
interface Props {
  ...
  isPaused?: boolean
  devMode?: boolean
  // Full map of equations in the recipe, used to distinguish pending rows
  // that are ready-but-waiting-on-the-concurrency-cap ("Queued") from rows
  // still blocked on upstream work.
  equationsByKey?: Map<string, RecipeEquation> | null
}
...
const isEchoed = computed<boolean>(() =>
  recipeRefs.hoveredRefKey.value === props.equation.equation_key,
)
...
// computing state as the recipe runs, and auto-expanding on that transition
