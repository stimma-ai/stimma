<template>
  ...
  >
    ...
    <!-- Right-pinned action footer. Mirrors HitlActionCard's button
         vocabulary so a recipe with mixed media-grid + text-row
...
interface Props {
  approveEquation: RecipeEquation | null
...
const pendingApproveTask = computed<RecipeTask | null>(() => {
...
const candidateProducer = computed<RecipeEquation | null>(
