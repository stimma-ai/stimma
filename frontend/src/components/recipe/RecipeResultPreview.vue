<template>
  ...
  <div class="text-[12.5px] leading-snug text-content">
    ...
    <template v-else-if="primary.kind === 'string'">
      <div
        v-if="renderMarkdown"
        class="recipe-result-markdown break-words"
...
// the dominant pattern for hitl.approve / image-gen recipes; the rest
...
<style scoped>
.recipe-result-markdown :deep(p) {
  margin: 0 0 0.65em;
}
.recipe-result-markdown :deep(p:last-child) {
  margin-bottom: 0;
}
.recipe-result-markdown :deep(strong) {
  font-weight: 700;
}
.recipe-result-markdown :deep(ul),
.recipe-result-markdown :deep(ol) {
  margin: 0.5em 0;
  padding-left: 1.35em;
}
.recipe-result-markdown :deep(ul) {
...
.recipe-result-markdown :deep(code) {
