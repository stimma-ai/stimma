  ...
  >
    ...
    <div class="flex-1 min-h-0 flex">
      ...
      >
        ...
        <!-- Error task: TaskCard handles retry/skip/edit-recipe -->
        <TaskCard
          v-if="errorTask"
          :task="errorTask"
          :dev-mode="devMode"
        ...
        <template v-else-if="focusedSlotIteration">
          ...
          <div v-else-if="slotCandidateValue !== undefined && slotCandidateValue !== null" class="rounded-md border border-edge-subtle bg-overlay-subtle/40 px-3 py-2">
            <RecipeResultPreview :value="slotCandidateValue" :max-lines="10" overflow-mode="none" />
          </div>
        ...
        <template v-else-if="standaloneHitlApprove">
          ...
          <div v-else-if="standaloneHitlApprove.value !== undefined" class="rounded-md border border-edge-subtle bg-overlay-subtle/40 px-3 py-2">
            <RecipeResultPreview :value="standaloneHitlApprove.value" :max-lines="10" overflow-mode="none" />
          </div>
        </template>

        <!-- ============= Recipe input: editable single-field form ============= -->
        ...
        <template v-else-if="focusEquation?.equation_type === 'recipe_input' && singleFieldSchema">
          <div v-if="panelInputDirty" class="flex items-center justify-end gap-1.5">
            ...
            <button
              ...
              title="Apply this input value to the recipe"
        ...
        <!-- ============= Recipe input (fallback) / output: render value ============= -->
        <!-- Used when the schema isn't editable here (literal_list, schema
             not yet loaded) and for output nodes (always read-only). -->
        <template v-else-if="focusEquation && (focusEquation.equation_type === 'recipe_input' || focusEquation.equation_type === 'recipe_output')">
          ...
          <div v-else-if="inputValue !== undefined && inputValue !== null" class="rounded-md border border-edge-subtle bg-overlay-subtle/40 px-3 py-2">
            <RecipeResultPreview :value="inputValue" :max-lines="14" overflow-mode="none" />
          </div>
          <div v-else class="text-[12px] text-content-muted italic">
            No value yet.
          </div>
...
import RecipeInputForm from './RecipeInputForm.vue'
...
interface Props {
  ...
  // Recipe-level input schema + current values, forwarded so the panel can
  // offer a one-field editable form when a recipe_input node is selected —
...
const emit = defineEmits<{
  ...
  // Partial values map (one key, the selected input field). RecipeView
  // merges it onto the existing recipe.inputs blob — the form here only
...
// Hover-echo: when the user hovers a recipe-ref chip in the chat context
...
// key resolution stays in lockstep with the panel's own RecipeRefButton.
...
const isPanelEchoed = computed<boolean>(() => {
  ...
  return recipeRefs.hoveredRefKey.value === target.key
})

// ---- Selection resolution ----
const focusEquation = computed<RecipeEquation | null>(() => {
...
const slotCandidate = computed<RecipeEquation | null>(() => {
...
const slotApproveEquation = computed<RecipeEquation | null>(() => {
  const it = focusedSlotIteration.value
  if (!it) return null
  return it.equations.find((e) => e.equation_type === 'hitl' && e.hitl_kind === 'approve') ?? null
})
const slotPendingApproveTask = computed<RecipeTask | null>(() => {
...
const hitlTask = computed<RecipeTask | null>(() => {
...
const errorTask = computed<RecipeTask | null>(() => {
  const eq = focusEquation.value
...
function toolTitle(eq: RecipeEquation): string {
...
function equationTitle(eq: RecipeEquation): string {
  switch (eq.equation_type) {
    ...
    case 'recipe_input': return eq.display_name || 'Input'
    case 'recipe_output': return eq.display_name || 'Output'
...
const iconBgClass = computed<string>(() => {
  ...
  switch (eq?.equation_type) {
    ...
    case 'recipe_input': return 'bg-gradient-to-br from-zinc-500/80 to-zinc-700/80'
    case 'recipe_output': return 'bg-gradient-to-br from-pink-500/80 to-pink-700/80'
...
const iconPath = computed<string>(() => {
  ...
  switch (eq?.equation_type) {
    ...
    case 'recipe_input': return INPUT_ICON
    case 'recipe_output': return OUTPUT_ICON
...
async function showGenerationDetails() {
  ...
  const eq = focusEquation.value
  if (!eq || props.recipeId == null) return
  detailsTrace.value = null
  detailsLoading.value = true
  try {
    const url = `${getApiBase()}/recipes/${props.recipeId}/equations/${encodeURIComponent(eq.equation_key)}/trace`
...
const errorMessage = computed<{ title: string; body: string } | null>(() => {
  ...
  const parsed = parseRecipeError(eq.error)
...
// Recipe input/output value: use the equation's `result` directly.
const inputValue = computed<unknown>(() => focusEquation.value?.result ?? null)

// Editable single-field input form: when the user selects a recipe_input
// node and the recipe carries a matching schema field, the panel renders
// the same RecipeInputForm the steps-view input card uses, scoped to that
...
const editableInputName = computed<string | null>(() => {
  const eq = focusEquation.value
  if (!eq || eq.equation_type !== 'recipe_input') return null
...
function onPanelInputSubmit(values: Record<string, any>) {
  // RecipeInputForm emits the full schema slice — for our one-field schema
  // that's already the partial we want to merge upstream.
  emit('update-inputs', values)
}
const panelInputFormRef = ref<InstanceType<typeof RecipeInputForm> | null>(null)
const panelInputDirty = ref(false)
const panelInputValid = ref(true)
// Reset dirty/valid when the selected input field changes — RecipeInputForm
...
// else has a panel-specific treatment above (recipe input/output, info,
