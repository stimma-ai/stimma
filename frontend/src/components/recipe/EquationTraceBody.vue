...
const parsedTraceError = computed(() =>
  parseRecipeError(trace.value?.error || props.equation.error),
)
...
// ---- Trace fetch ----
//
// Skip types whose useful data lives on the equation already (recipe_input/
...
const SKIP_TRACE_TYPES = new Set([
  'recipe_input', 'recipe_output', 'control', 'llm_batch', 'info', 'hitl',
])
