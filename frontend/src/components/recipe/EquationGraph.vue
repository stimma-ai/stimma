  ...
  <div class="w-full h-full overflow-hidden relative select-none bg-base">
    ...
    <div v-if="layoutNodes.length === 0" class="absolute inset-0 flex items-center justify-center">
      <div class="text-center space-y-1.5">
        ...
        <div class="text-[12px] text-content-muted">Start the recipe to see how its steps connect.</div>
      </div>
    </div>
...
const props = withDefaults(defineProps<{
  ...
  // against the correct recipe. Null when the view hasn't loaded one yet.
  recipeId?: number | string | null
  // Forwarded into GraphInspectPanel so selecting a recipe_input node in
...
const INSPECT_PANEL_HEIGHT_KEY = makeProfileKey('recipe_graph', 'inspect_panel_height')
const INSPECT_PANEL_OPEN_KEY = makeProfileKey('recipe_graph', 'inspect_panel_open')
...
// the selected node disappears (e.g. recipe re-parsed).
...
// Persisted per-recipe so flipping tabs / reloading keeps the user on the
...
// renamed by a recipe re-parse).
const selectionKey = computed<string | null>(() => {
  if (props.recipeId == null) return null
  return makeProfileKey('recipe_graph', String(props.recipeId), 'selection')
})
...
// Restore on recipeId arrival/change so each recipe gets its own spot.
...
const focusedIterationEquation = computed<RecipeEquation | null>(() => {
...
const inspectEquation = computed<RecipeEquation | null>(() => {
...
interface LayoutNode {
  ...
  // Synthetic "recipe_output" nodes mirror a real equation's state. source_key
...
function isHiddenIterationWrapper(eq: RecipeEquation): boolean {
...
function buildLayout() {
  ...
  const inputKeys = new Set(eqs.filter(e => e.equation_type === 'recipe_input').map(e => e.equation_key))
  function nodeSizeFor(eq: RecipeEquation): { w: number; h: number } {
  ...
  // references recipe inputs via string interpolation rather than NodeRefs)
  ...
  // Synthetic recipe_output nodes — the mirror image of recipe_input. Drawn
  // for equations the recipe's `return` actually surfaces (backend sets
  ...
  // Pin recipe_input nodes to the leftmost column — dagre's rank minimization
...
const layoutEdges = computed<LayoutEdge[]>(() => {
  ...
  // Synthetic edges (source equation → recipe_output node) feed the same port
...
function nodeTitle(n: LayoutNode): string {
  switch (n.equation_type) {
    ...
    case 'recipe_input': return n.display_name || shortKey(n.key)
    ...
    case 'recipe_output': return n.display_name || 'Output'
    case 'control': {
...
//   recipe_input → none (the INPUT chip already says it)
//   recipe_output → producing step's title
function nodeSubtitle(n: LayoutNode): string | null {
  ...
  if (n.equation_type === 'recipe_input') {
    return null
  }
  if (n.equation_type === 'recipe_output') {
...
// already give. Match against the same key the footer RecipeRefButton emits,
...
function isNodeEchoed(n: LayoutNode): boolean {
  return recipeRefs.hoveredRefKey.value === nodeRefKey(n)
}
...
function statusLabel(n: LayoutNode): string {
  ...
  if (n.equation_type === 'recipe_output' && n.status === 'pending') {
    return 'waiting'
  }
...
function edgeStroke(srcEq: RecipeEquation | undefined): string {
  if (srcEq && (srcEq.result_media_ids?.length ?? 0) > 0) {
    return 'var(--color-recipe-edge-media)'
  }
  switch (srcEq?.equation_type) {
    case 'tool_call':    return 'var(--color-recipe-edge-tool)'
    case 'llm_call':
    case 'llm_batch':
    case 'llm_slot':     return 'var(--color-recipe-edge-llm)'
    case 'code':         return 'var(--color-recipe-edge-code)'
    case 'hitl':         return 'var(--color-recipe-edge-hitl)'
    case 'info':         return 'var(--color-recipe-edge-info)'
    case 'recipe_input': return 'var(--color-recipe-edge-input)'
    case 'control':      return 'var(--color-recipe-edge-control)'
...
function outputDisplayName(eq: RecipeEquation, fallback: string | null = null): string {
