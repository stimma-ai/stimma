<template>
  <div class="flex flex-col h-full min-h-0 min-w-0 text-content" :class="['image', 'video'].includes(kind) ? 'bg-matte' : 'bg-base'" @click.stop>
    <Teleport :to="controlsTarget || 'body'" :disabled="!controlsTarget">
    <div v-if="kind === 'table' && rows.length" class="flex items-center gap-2 px-3 py-1 shrink-0">
    ...
    <WorkspaceZipViewer v-else-if="kind === 'zip' && workspaceFile && chatId != null" :file="workspaceFile" :chat-id="chatId" :header-target="controlsTarget" @attach="$emit('attach', $event)" @save="$emit('save', $event)" />
    <ReadOnlyCode v-else-if="kind === 'text' || kind === 'json'" :text="text" :name="name" class="flex-1" />
...
const text = ref(''), loading = ref(false), error = ref('')
...
watch(() => props.url, async (url, _, cleanup) => {
  ...
  text.value = ''; error.value = ''; scrollTop.value = 0; loading.value = false
