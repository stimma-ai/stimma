 ...
 * The group derivation is pure: inputs are `RecipeEquation`s, output is a
 * render-order array. The rules come straight from the equation-key format
 * (see backend/recipe_runtime/keys.py):
 *
...
interface Options {
  isEquationActionable?: (eq: RecipeEquation) => boolean
...
function aggregateDuration(equations: RecipeEquation[]): number | null {
...
export function buildIterationsForForeach(
  ...
  allEquationsByKey: Map<string, RecipeEquation>,
  opts: { isEquationActionable?: (eq: RecipeEquation) => boolean } = {},
): GroupedIteration[] {
  const isActionable = opts.isEquationActionable || (() => false)
  const prefix = foreach.equation_key + '/'
  const wrappers: RecipeEquation[] = []
...
export function groupEquationsForPhase(
  ...
  const wrappers: RecipeEquation[] = []
  const foreachByKey = new Map<string, RecipeEquation>()
  const approveByKey = new Map<string, RecipeEquation>()
  for (const eq of allEquationsByKey.values()) {
  ...
  const wrappersByGroup = new Map<string, RecipeEquation[]>()
  for (const w of wrappers) {
...
//
// For the user's `tool() → hitl.approve()` recipe, this produces:
...
function buildSlotGroupItems(
  ...
  // The agent writes per-recipe `instructions=` on `hitl.approve(...)`, but
  ...
  for (const sub of subSteps) {
    ...
    const totalDurationMs = aggregateDuration(
      sub.equations.filter((e): e is RecipeEquation => e != null),
    )
    ...
    const classifyPrimaries: (RecipeEquation | null)[] =
...
// a result lands.
export function candidateProducerForSlot(slot: GroupedIteration): RecipeEquation | null {
...
function subStepDisplayName(sub: SlotSubStep, approve: RecipeEquation): string {
...
function buildSubSteps(
  ...
  const byPath = new Map<string, (RecipeEquation | null)[]>()
  ...
  return orderedPaths.map((relPath) => {
    ...
    const rep = equations.find((e): e is RecipeEquation => e != null) ?? null
