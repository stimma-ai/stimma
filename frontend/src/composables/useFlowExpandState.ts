...
/**
 * Sticky expand/collapse state for flow step rows and iteration groups.
 *
 ...
 * flowId so forks and parallel flows don't cross-contaminate.
 *
...
function stateKey(flowId: number | string | null, rowKind: RowKind, id: string): string {
  return `${flowId ?? 'global'}:${rowKind}:${id}`
}

// Keyed by stateKey() → explicit user override. Absence means "use default".
const overrides = reactive(new Map<string, boolean>())

export function useFlowExpandState(flowIdRef: ComputedRef<number | string | null> | { value: number | string | null }) {
  function getOverride(rowKind: RowKind, id: string): boolean | undefined {
    return overrides.get(stateKey(flowIdRef.value, rowKind, id))
  }
  function setOverride(rowKind: RowKind, id: string, value: boolean): void {
    overrides.set(stateKey(flowIdRef.value, rowKind, id), value)
  }
