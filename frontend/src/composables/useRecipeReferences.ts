...
// Provided by RecipeView at the top of its subtree so every phase-tree row
// (PhaseNode, EquationTraceRow, IterationGroup, IterationCard) can resolve
// the current recipe's chat scope without prop-drilling through four levels.
export const RECIPE_CHAT_ID_KEY: InjectionKey<Ref<string | number | null>> =
  Symbol('recipeChatId')

export function injectRecipeChatIdRef(): Ref<string | number | null> {
  const r = inject(RECIPE_CHAT_ID_KEY, null as unknown as Ref<string | number | null>)
...
// Kinds match the granularities the recipe UI exposes: a whole phase, a
...
export type RecipeRefKind = 'phase' | 'equation' | 'iteration-group' | 'iteration'
...
function bucketFor(chatId: string | number | null): ChatBucket {
  ...
  if (!b) {
    b = { items: ref<RecipeReference[]>([]), hoveredRefKey: ref<string | null>(null) }
    buckets.set(key, b)
  }
  return b
}

export interface UseRecipeReferences {
  items: ComputedRef<RecipeReference[]>
  hoveredRefKey: ComputedRef<string | null>
  count: ComputedRef<number>
  has: (refKey: string) => boolean
  add: (ref: RecipeReference) => void
  remove: (refKey: string) => void
  toggle: (ref: RecipeReference) => void
  clear: () => void
...
// (e.g. phase-tree rows whose chat scope is set by RecipeView after async
...
export function useRecipeReferences(chatIdArg: ChatIdArg): UseRecipeReferences {
  ...
  const items = computed<RecipeReference[]>(() => getBucket().items.value)
  ...
  function add(r: RecipeReference) {
    const b = getBucket()
  ...
  function toggle(r: RecipeReference) {
...
// messages so the agent sees which recipe items the user was pointing at.
...
export function formatReferencesForMessage(refs: RecipeReference[]): string {
...
function headerLabelToKind(label: string): RecipeRefKind {
  switch (label) {
    case 'Phase': return 'phase'
