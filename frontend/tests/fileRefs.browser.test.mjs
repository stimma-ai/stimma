...
for (const phone of [false, true]) test(`file refs stage and round trip (${phone ? 'phone' : 'desktop'})`, async t => {
  ...
  await expect.poll(() => page.evaluate(() => window.fileRefsBack.enabled())).toBe(false)
  await chip('resize_batch.py').click()
  ...
  await expect(page.getByRole('button', { name: /^(Source|Raw|Preview)$/ })).toHaveCount(0)
  await close()
  ...
  await page.locator('button[aria-label="hero.png"]').click()
  ...
  await page.getByRole('option', { name: 'Smallest first', exact: true }).click()
  const browserEntries = page.locator('button[draggable="true"][aria-label]')
  await expect(browserEntries.first()).toHaveAttribute('aria-label', 'notes.md')
  await page.getByRole('button', { name: 'Smallest first', exact: true }).click()
  await page.getByRole('option', { name: 'Largest first', exact: true }).click()
  await expect(browserEntries.first()).toHaveAttribute('aria-label', 'hero.png')
  await page.screenshot({ path: `/tmp/file-refs-${phone ? 'phone' : 'desktop'}-browser.png` })
  ...
  await expect(page.locator('.cm-content')).toContainText('images')
  ...
  if (!phone) {
    ...
    await page.locator('[draggable="true"]').filter({ hasText: 'manifest.json' }).first().dispatchEvent('dragstart', { dataTransfer: transfer })
    await page.getByPlaceholder('Type a message...').dispatchEvent('drop', { dataTransfer: transfer })
    await expect.poll(() => page.evaluate(() => window.fileRefsTest.inputAttachments.length)).toBe(2)
    await page.getByRole('button', { name: 'Remove attachment' }).last().click()
    await chip('report.md').locator('..').dispatchEvent('dragstart', { dataTransfer: transfer })
    await page.getByPlaceholder('Type a message...').dispatchEvent('drop', { dataTransfer: transfer })
    await expect.poll(() => page.evaluate(() => window.fileRefsTest.inputAttachments.length)).toBe(2)
    await page.getByRole('button', { name: 'Remove attachment' }).last().click()
  }
  ...
  await expect(page.getByRole('button', { name: /^(Source|Raw|Preview)$/ })).toHaveCount(0)
  assert.equal(await page.evaluate(() => window.fileRefsBack.consume()), true)
  await expect.poll(() => page.evaluate(() => window.fileRefsTest.artifactStage.stageOpen.value)).toBe(false)
  assert.equal(await page.evaluate(() => window.fileRefsBack.consume()), false)
  if (phone) {
